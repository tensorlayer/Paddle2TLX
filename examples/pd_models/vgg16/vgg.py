...
class VGG(nn.Layer):
    ...
    def forward(self, x):
        # print(self.features[0](x).shape)  # np.array(self.features[0](x))
        # print(self.features[0](x))
        x = self.features(x)
        # np.save('tmp/pd_feature.npy', np.array(x))
        if self.with_pool:
            x = self.avgpool(x)
...
def make_layers(cfg, batch_norm=False):
    ...
    for v in cfg:
        if v == 'M':
            layers += [nn.MaxPool2D(kernel_size=2, stride=2)]  # padding: 0(默认)|VALID, 1|SAME
        else:
            conv2d = nn.Conv2D(in_channels, v, kernel_size=3, padding=1)  # padding: 0(默认)|VALID, 1|SAME
            if batch_norm:
...
def _vgg(arch, cfg, batch_norm, pretrained, **kwargs):
    ...
    if pretrained:
        model = load_model(model, arch, model_urls)
