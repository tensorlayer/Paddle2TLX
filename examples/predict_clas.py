...
image_file = os.path.join(os.path.dirname(__file__), "images/dog.jpeg")
file_path = os.path.join(os.path.dirname(__file__), 'images/imagenet_classes.txt')


def predict_pd(model, model_name="vgg16"):
    """ paddlepaddle sample prediction """
    import paddle

    print("=" * 16, "Top5 prediction category - PaddlePaddle", "=" * 16)
    model.eval()
...
def predict_tlx(model, model_name="vgg16"):
    """ tensorlayerx sample prediction """
    import paddle
    import tensorlayerx as tlx

    print("="*16, "Top5 prediction category - TensorLayerX", "="*16)
    model.set_eval()
    print('Model name:', model_name)
...
def calc_diff(model_tlx, model_pd, model_name="vgg16"):
