...
# from ops.tlx_basic_pooling import tlx_AvgPool2d
# from ops.tlx_extend import get_tensor_shape
from paddle2tlx.pd2tlx.utils import load_model_cdet
STANet_URLS = 'https://paddlers.bj.bcebos.com/pretrained/cd/levircd/weights/stanet_levircd.pdparams'
...
class STANet(nn.Layer):
    ...
    def forward(self, t1, t2):
        f1 = self.extract(t1)
        # print(f"tlx self.f1.shape={f1.shape}")
        f2 = self.extract(t2)
        # print(f"tlx self.f2.shape={f2.shape}")
...
class Backbone(nn.Layer, KaimingInitMixin):
    ...
    def forward(self, x):
        ...
        # print(f"self.resnet.conv1.x.shape={x.shape}")
        x = self.resnet.bn1(x)
        x = self.resnet.relu(x)
        x = self.resnet.maxpool(x)
        # print(f"self.resnet.maxpool.x.shape={x.shape}")
...
class BAM(nn.Layer):
    def __init__(self, in_ch, ds):
        ...
        self.ds = ds
        # self.pool = nn.AvgPool2D(self.ds)  # todo
        self.pool = self.init_pd(self.ds)  # add
    ...
    def init_pd(self, input):
        pool = nn.AvgPool2D(input)
        return pool

    def init_tlx(self, input):
        pool = nn.AvgPool2D(input, stride=1)
        return pool

    def forward(self, x):
        x = x.flatten(-2)
        x_rs = self.pool(x)
        # print(f"BAM.x.shape={x.shape}")
        ...
        # print(f"BAM.out.shape={out.shape}")
        out = out + x
...
def _stanet(pretrained=None, in_channels=3, num_classes=2):
    ...
    if pretrained:
        model = load_model_cdet(model, STANet_URLS, "stanet")
        # weight_path = download_and_decompress(STANet_URLS)
