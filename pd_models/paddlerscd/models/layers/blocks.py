...
# from ops.tlx_padding import tlx_Pad2d
...
class BasicConv(nn.Layer):
    def __init__(self,
        ...
        # if b_init is False:  # comment
        #     b_init = None
        # else:
        #     b_init = "constant"
        b_init = self.init_pd(b_init)  # todo
        seq.append(
            nn.Conv2D(
                in_channels=in_ch,
                out_channels=out_ch,
        ...
        #     seq.append(norm)
        if norm:
            seq.append(nn.BatchNorm2D(num_features=out_ch))
        # if act:
        #     if act is True:
        #         act = make_act()
        #     seq.append(act)
        ...
        self.seq = self.get_sequential_pd(seq)
...
# class MaxPool2x2(tlx_MaxPool2d):  # todo
...
class ConvTransposed3x3(nn.Layer):
    def __init__(self,
        ...
        # bias_attr = (False if norm else None) if bias == 'auto' else bias
        b_init = None
        b_init = self.init_pd(b_init, bias, norm)
        seq.append(
            # nn.Conv2DTranspose(
            #     in_ch,
            #     out_ch,
