...
DSAMNet_URLS = 'https://paddlers.bj.bcebos.com/pretrained/cd/levircd/weights/dsamnet_levircd.pdparams'
...
class DSLayer(nn.Sequential):
    def __init__(self, in_ch, out_ch, itm_ch, **convd_kwargs):
        super(DSLayer, self).__init__(
            ...
            nn.Conv2DTranspose(in_channels=in_ch,
                               out_channels=itm_ch,
                               kernel_size=3,
                               padding=1,
                               **convd_kwargs),  # todo
            # make_norm(itm_ch),
            nn.BatchNorm2D(itm_ch),  # todo
            nn.ReLU(),
            nn.Dropout2D(p=0.2),
            # nn.Conv2DTranspose(
            #     itm_ch, out_ch, kernel_size=3, padding=1))
            nn.Conv2DTranspose(in_channels=itm_ch,
                               out_channels=out_ch,
                               kernel_size=3,
                               padding=1))
...
def _dsamnet(pretrained=None, in_channels=3, num_classes=2):
    ...
    if pretrained:
        model = load_model_cdet(model, DSAMNet_URLS, "dsamnet")
        # weight_path = download_and_decompress(DSAMNet_URLS)
