...
# from ops.tlx_activation import tlx_GELU
from .param_init import KaimingInitMixin
# from utils.download import download_and_decompress
from paddle2tlx.pd2tlx.utils import load_model_cdet

BIT_URLS = "https://paddlers.bj.bcebos.com/pretrained/cd/levircd/weights/bit_levircd.pdparams"
...
class CrossAttention(nn.Layer):
    def __init__(self,
        ...
        #     # nn.Dropout(dropout_rate)
        #     )
        self.fc_out = self.get_sequential_pd(inner_dim, dim)
    ...
    def forward(self, x, ref):
        ...
        v_tmp = paddle.matmul(q, k, transpose_y=True)
...
def _bit(pretrained=None, in_channels=3, num_classes=2):
    ...
    if pretrained:
        model = load_model_cdet(model, BIT_URLS, "bit")
        # weight_path = download_and_decompress(BIT_URLS)
        # param = paddle.load(weight_path)
        # new_weights = get_new_weights(param)
        # # print(len([k for k in get_new_weights.keys()]))
