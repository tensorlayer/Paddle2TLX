...
# from ops.tlx_basic_conv import tlx_ConvTranspose2d  # todo
from .param_init import KaimingInitMixin
# from ops.tlx_basic_sample import tlx_Upsample
# from utils.download import download_and_decompress
from paddle2tlx.pd2tlx.utils import load_model_cdet
SNUNET_URLS = 'https://paddlers.bj.bcebos.com/pretrained/cd/levircd/weights/snunet_levircd.pdparams'
...
class Up(nn.Layer):
    def __init__(self, in_ch, use_conv=False):
        ...
        if use_conv:
            ...
            self.up = nn.Conv2DTranspose(in_channels=in_ch, out_channels=in_ch, kernel_size=2, stride=2)
        else:
            self.up = nn.Upsample(
...
def _snunet(pretrained=None, in_channels=3, num_classes=2):
    ...
    if pretrained:
        model = load_model_cdet(model, SNUNET_URLS, "snunet")
        # weight_path = download_and_decompress(SNUNET_URLS)
