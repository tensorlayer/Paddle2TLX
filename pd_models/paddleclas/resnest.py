...
from paddle.nn.initializer import Uniform  # add
from paddle.nn.initializer import KaimingNormal
from paddle.nn import Conv2D, BatchNorm, Linear#, Dropout
from paddle.nn import AdaptiveAvgPool2D#, MaxPool2D, AvgPool2D
from paddle.regularizer import L2Decay
from collections import OrderedDict
# from ops.tlx_extend import tlx_AvgPool2d, tlx_MaxPool2d
...
class rSoftmax(nn.Layer):
    ...
    def forward(self, x):
        ...
        if self.radix > 1:
            ...
            x = paddle.transpose(x, perm=[0, 2, 1, 3])  # x=x -> x
            x = nn.functional.softmax(x, axis=1)
            x = paddle.reshape(x, shape=[batch, r * h * w, 1, 1])
        else:
