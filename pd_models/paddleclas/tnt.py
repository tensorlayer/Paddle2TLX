...
# from ops.tlx_extend import tlx_Dropout, get_tensor_shape
...
class Mlp(nn.Layer):
    def __init__(self,
        ...
        self.act = act_layer()
        # if act_layer == paddle.nn.GELU:
        #     self.act = act_layer()
        # elif act_layer == tensorlayerx.ops.gelu:
        #     self.act = act_layer
...
class Attention(nn.Layer):
    def __init__(self,
        ...
        # b_init = None if qkv_bias is False else "constant"  # todo
        # self.qk = nn.Linear(dim, hidden_dim * 2, bias_attr=b_init)
        # self.v = nn.Linear(dim, dim, bias_attr=b_init)
        b_init = None
        b_init = self.init_pd(b_init, qkv_bias)
        self.qk = nn.Linear(dim, hidden_dim * 2, bias_attr=b_init)
        self.v = nn.Linear(dim, dim, bias_attr=b_init)
...
class Block(nn.Layer):
    def __init__(self,
        ...
        # self.norm1_proj = layer_norm(dim)  # todo
        self.proj = nn.Linear(in_dim * num_pixel, dim)
        # b_init = None if qkv_bias is False else "constant"  # todo
        # self.proj = nn.Linear(in_dim * num_pixel, dim, bias_attr=b_init)
        # Outer transformer
...
class PixelEmbed(nn.Layer):
    ...
    def forward(self, x, pixel_pos):
        ...
        # x = x.transpose((0, 2, 1)).reshape(
        #     (-1, self.new_patch_size * self.new_patch_size, self.in_dim))  # todo
        x = x + pixel_pos
...
class TNT(nn.Layer):
    def __init__(self,
        ...
        # self.pixel_pos = self.create_parameter(
        #     shape=(1, new_patch_size * new_patch_size, in_dim),
        #     default_initializer=zeros_)  # todo
...
def _tnt(arch, pretrained=False, **kwargs):
    if arch == "tnt_small":
        model = TNT(patch_size=16,
                    ...
                    **kwargs)
    if pretrained:
        load_model_clas(model, arch, MODEL_URLS)
    return model
...
#          qkv_bias,
#          **kwargs):
#     model = TNT(patch_size, embed_dim, in_dim, depth, num_heads, in_num_head, qkv_bias, **kwargs)
#     if pretrained:
#         _load_pretrained(model, arch, MODEL_URLS)
#     return model
# 
# 
# def tnt_small(pretrained=False, **kwargs):
#     model = _tnt("tnt_small", 
#                  pretrained,
#                  patch_size=16,
#                  embed_dim=384,
#                  in_dim=24,
