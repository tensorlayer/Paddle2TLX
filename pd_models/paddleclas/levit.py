...
class BN_Linear(nn.Sequential):
    def __init__(self, a, b, bias=True, std=0.02):
        ...
        b_init = None
        b_init = self.init_pd(b_init, bias)
        l = nn.Linear(a, b, bias_attr=b_init)
...
class Attention(nn.Layer):
    ...
    def forward(self, x):
        ...
        ma = paddle.matmul(q, k_transpose)
        mas = ma * self.scale
...
def _levit(arch, class_num, distillation, pretrained, **kwargs):
    model = model_factory(**specification[arch], class_num=class_num, distillation=distillation)
    if pretrained:
