...
def drop_path(x, drop_prob=0., training=False):
    ...
    r = paddle.rand(shape).astype(x.dtype)
...
class CSwinStage(nn.Layer):
    ...
    def forward(self, x):
        for block in self.blocks:
            x = block(x)
            # print(x.sum(), '1222222222222222222222222222')
        x = self.merge(x)
        # print(x.sum(), '1333333333333333333333333333333')
        return x
...
class CSwinTransformer(nn.Layer):
    ...
    def forward_features(self, x):
        ...
        # print(x.sum(), '0000000000000000')
        for stage in self.stages:
            x = stage(x)
            # print(x.sum(), '1111111111111111')
        x = self.norm(x)
...
def CSwintransformer_thiny(pretrained=False, **kwargs):
    return _CSwintransformer_thiny("CSWinTransformer_tiny_224", pretrained, **kwargs)
...
#
# def CSwintransformer_thiny(pretrained=False, **kwargs):
#     model = CSWinTransformer_tiny_224(pretrained=pretrained, **kwargs)
#     return model