...
def data_process(data):
    ...
    for batch_id, d in enumerate(data):
        ...
        padded_sents.append(paddle.to_tensor(padded_sent))
...
def pd_tlx_diff(pd_model, tlx_model, data):
    ...
    print(f'Model {pd_model.__class__.__name__} predict category - TLX:', label_map[np.argmax(pd_results)])
    print(f'Model {tlx_model.__class__.__name__} predict category - Paddle:', label_map[np.argmax(tlx_results)])
...
def tlx_f(model_tlx, data):
    ...
    for batch_id, data in enumerate(tests):
        ...
        tlx_out = paddle.nn.Softmax()(tlx_results[0]).numpy()
