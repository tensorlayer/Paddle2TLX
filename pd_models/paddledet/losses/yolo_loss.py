...
from models.bbox_utils import decode_yolo, xywh2xyxy, batch_iou_similarity
...
class YOLOv3Loss(nn.Layer):
    ...
    def cls_loss(self, pcls, tcls):
        if self.label_smooth:
            ...
            aa = paddle.cast(tcls > 0., dtype=tcls.dtype)
            bb = paddle.cast(tcls <= 0., dtype=tcls.dtype)
            tcls = pos * aa + neg * bb
    ...
    def yolov3_loss(self, p, t, gt_box, anchor, downsample, scale=1.,
        ...
        sx = F.sigmoid(x)
        sy = F.sigmoid(y)
        x = scale * sx - 0.5 * (scale - 1.)
        y = scale * sy - 0.5 * (scale - 1.)
