...
def quality_focal_loss(pred, target, beta=2.0, use_sigmoid=True):
    ...
    aa = paddle.logical_not(pos_mask)
...
def distribution_focal_loss(pred, label):
    ...
    loss1 = F.cross_entropy(pred, dis_left, reduction='none')
    loss2 = F.cross_entropy(pred, dis_right, reduction='none')
    loss = loss1 * weight_left \
        + loss2 * weight_right
    return loss
