...
class DIouLoss(GIoULoss):
    ...
    def __call__(self, pbox, gbox, iou_weight=1.):
        ...
        aa = paddle.greater_than(xkis2, xkis1)
        bb = paddle.greater_than(ykis2, ykis1)
        ...
        if self.use_complete_iou_loss:
            ar_gt = wg / hg
            ar_pred = w / h
            aa = paddle.atan(ar_gt)
            bb = paddle.atan(ar_pred)
            arctan = aa - bb
...
class SIoULoss(GIoULoss):
    ...
    def __call__(self, pbox, gbox):
        ...
        aa = paddle.maximum(cy, cyg)
        bb = paddle.minimum(cy, cyg)
        cc = paddle.maximum(cx, cxg)
        dd = paddle.minimum(cx, cxg)
        ch = aa - bb
        cw = cc - dd
        ...
        aa = paddle.pow(paddle.to_tensor(2), 0.5)
        thred = aa / 2
        ...
        aa = paddle.exp(-gamma * beta_x)
        bb = paddle.exp(-gamma * beta_y)
        dist_cost = 1 - aa + 1 - bb

        # shape cost
        aa = paddle.abs(w - wg)
        bb = paddle.abs(hg - h)
        cc = paddle.maximum(w, wg)
        dd  = paddle.maximum(h, hg)
        omega_w = aa / cc
        omega_h = bb / dd
        aa = paddle.exp(-omega_w)
        bb = paddle.exp(-omega_h)
