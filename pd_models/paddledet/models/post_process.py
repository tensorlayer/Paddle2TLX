...
__all__ = [
    'BBoxPostProcess', 'MaskPostProcess', 'FCOSPostProcess', 'CenterNetPostProcess', 'SparsePostProcess'
]
...
class MaskPostProcess(object):
    ...
    def paste_mask(self, masks, boxes, im_h, im_w):
        ...
        N = masks.shape[0]
        aa = paddle.arange(y0_int, y1_int)
        bb = paddle.arange(x0_int, x1_int)
        img_y = aa + 0.5
        img_x = bb + 0.5
    ...
    def __call__(self, mask_out, bboxes, bbox_num, origin_shape):
        ...
        if self.export_onnx:
            ...
            aa = paddle.zeros([num_mask, max_h, max_w], dtype='int32')
            pred_result = aa - 1
...
class SparsePostProcess(object):
    ...
    def __call__(self, box_cls, box_pred, scale_factor_wh, img_whwh):
        ...
        for i in range(len(scale_factor_wh)):
            ...
            aa = paddle.clip(boxes[:, 0::2], min=0, max=img_wh[i][0])
            bb = paddle.clip(boxes[:, 1::2], min=0, max=img_wh[i][1])
