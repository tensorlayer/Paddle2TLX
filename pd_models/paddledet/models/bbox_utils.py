...
def bbox2delta(src_boxes, tgt_boxes, weights):
    ...
    aa = paddle.log(tgt_w / src_w)
    bb = paddle.log(tgt_h / src_h)
    dw = ww * aa
    dh = wh * bb
...
def batch_bbox_overlaps(bboxes1,
    ...
    if rows * cols == 0:
        if is_aligned:
            return paddle.full(shape=batch_shape + (rows, ), fill_value=1)
        else:
            return paddle.full(shape=batch_shape + (rows, cols), fill_value=1)
...
def bbox_iou(box1, box2, giou=False, diou=False, ciou=False, eps=1e-9):
    ...
    if giou or ciou or diou:
        # convex w, h
        aa = paddle.maximum(px2, gx2)
        bb = paddle.minimum(px1, gx1)
        cc = paddle.maximum(py2, gy2)
        dd = paddle.minimum(py1, gy1)
        cw = aa - bb
        ch = cc - dd
        if giou:
            c_area = cw * ch + eps
            ...
            if diou:
                ...
                aa = paddle.atan(w1 / h1)
                bb = paddle.atan(w2 / h2)
                delta = aa - bb
                aa = paddle.pow(delta, 2)
                v = (4 / math.pi**2) * aa
                alpha = v / (1 + eps - iou + v)
