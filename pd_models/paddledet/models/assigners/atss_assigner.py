    ...
    def forward(self,
        ...
        if num_max_boxes == 0:
            assigned_labels = paddle.full(
                shape=[batch_size, num_anchors], fill_value=bg_index, dtype='int32')
        ...
        a0 = paddle.full_like(x=assigned_labels, fill_value=bg_index, dtype=assigned_labels.dtype)
        b = mask_positive_sum > 0
        assigned_labels = paddle.where(
            mask_positive_sum > 0, assigned_labels,
            paddle.full_like(x=assigned_labels, fill_value=bg_index, dtype=assigned_labels.dtype))
