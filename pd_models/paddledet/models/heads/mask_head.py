...
from core.workspace import register, create
...
class MaskHead(nn.Layer):
    ...
    def forward_test(self,
        ...
        if not self.export_onnx and rois.shape[0] == 0:
            mask_out = paddle.full(shape=[1, 1, 1], fill_value=-1)
        else:
            bbox = [rois[:, 2:]]
            ...
            if self.num_classes == 1:
                ...
                # ])
                mask_out = paddle.reshape(mask_out, [
                    paddle.shape(index)[0], mask_out_shape[-2], mask_out_shape[-1]
                ])
