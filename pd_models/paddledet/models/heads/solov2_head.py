...
from models.layers import ConvNormLayer, MaskMatrixNMS, DropBlock
...
class SOLOv2MaskHead(nn.Layer):
    ...
    def __init__(self,
        ...
        for i in range(start_level, end_level + 1):
            ...
            if i == start_level:
                ...
                self.add_sublayer('conv_pre_feat' + str(i), nn.Sequential(*conv_pre_feat))  # *conv_pre_feat
                self.convs_all_levels.append(nn.Sequential(*conv_pre_feat))  # *conv_pre_feat
            else:
                for j in range(i):
                    ch_in = 0
                    if j == 0:
                    ...
                    conv_pre_feat.append(
                        ...
                        ConvNormLayer(
                            ...
                            norm_type=self.norm_type))
                    conv_pre_feat.append(
                        # conv_feat_name + '.conv' + str(j) + 'act', 
                        nn.ReLU())
                    conv_pre_feat.append(
                        # 'upsample' + str(i) + str(j),
                        nn.Upsample(
                            scale_factor=2, mode='bilinear'))
                self.add_sublayer('conv_pre_feat' + str(i), nn.Sequential(*conv_pre_feat))
    ...
    def forward(self, inputs):
        ...
        # print(f"feat_all_level.shape={feat_all_level.shape}")
        for i in range(1, self.range_level):
            input_p = inputs[i]
            # print(f"input_p.shape={input_p.shape}")
            ...
            # print(f"i={i}, feat_all_level.shape={feat_all_level.shape}")
            # print(f"self.convs_all_levels[i](input_p)={self.convs_all_levels[i](input_p).shape}")
...
class SOLOv2Head(nn.Layer):
    ...
    def get_seg_single(self, cate_preds, seg_preds, kernel_preds, featmap_size,
        ...
        ind_a = paddle.to_tensor(paddle.shape(kernel_preds)[0], 'int64')
        ...
        # Prevent empty and increase fake data
        aa = paddle.shape(sum_masks)
        keep_other = paddle.concat([keep, paddle.to_tensor(aa[0] - 1, 'int64')])
        keep_scores = paddle.concat([keep, paddle.to_tensor(paddle.shape(sum_masks)[0], 'int64')])
        ...
        aa = paddle.sum(seg_mul, axis=[1, 2])
