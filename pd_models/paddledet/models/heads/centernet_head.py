...
from losses import CTFocalLoss, GIoULoss
...
class ConvLayer(nn.Layer):
    def __init__(self,
        ...
        self.conv = nn.Conv2D(
            ...
            dilation=dilation,
            groups=groups,
            weight_attr=paddle.ParamAttr(initializer=Uniform(-bound, bound)),
            bias_attr=bias_attr)
...
class CenterNetHead(nn.Layer):
    ...
    def get_loss(self, inputs, weights, heatmap, size, offset, iou=None):
        ...
        heatmap = paddle.clip(F.sigmoid(heatmap), min=1e-4, max=1 - 1e-4)
