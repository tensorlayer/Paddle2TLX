...
class RPNHead(nn.Layer):
    ...
    def _gen_proposal(self, scores, bbox_deltas, anchors, inputs):
        ...
        if self.export_onnx:
            ...
            shape_im_shape = paddle.shape(im_shape)
            try:
                batch_size = paddle.slice(shape_im_shape, [0], [0], [1])
            except:
                batch_size = paddle.slice(paddle.to_tensor(shape_im_shape), [0], [0], [1])
    ...
    def get_loss(self, pred_scores, pred_deltas, anchors, inputs):
        ...
        scores = paddle.reshape(scores, shape=(-1, ))
        deltas = paddle.reshape(deltas, shape=(-1, 4))
        ...
        if valid_ind.shape[0] == 0:
            ...
            loss_rpn_cls = F.binary_cross_entropy_with_logits(
                score_pred, score_label, reduction="sum")
