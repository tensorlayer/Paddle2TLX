...
from models.bbox_utils import bbox2delta, delta2bbox
...
class RetinaHead(nn.Layer):
    ...
    def get_loss(self, head_outputs, targets):
        ...
        for matches, match_labels, cls_logit, bbox_reg, gt_bbox, gt_class in \
            ...
            matches = paddle.where(neg_mask,
                       paddle.full_like(x=matches, fill_value=gt_class.size - 1, dtype=matches.dtype),
                       matches)
    ...
    def get_bboxes_single(self,
        ...
        if rescale:
            aa = paddle.concat([scale_factor[::-1], scale_factor[::-1]])
