...
from models.bbox_utils import distance2bbox, bbox2distance, batch_distance2bbox
...
class Integral(nn.Layer):
    ...
    def forward(self, x):
        ...
        x = F.linear(x=x, weight=self.project)  # TODO
        if self.training:
            x = x.reshape([-1, 4])
        return x
...
class GFLHead(nn.Layer):
    ...
    def get_single_level_center_point(self, featmap_size, stride,
        ...
        """
        h, w = featmap_size
        aa = paddle.arange(w, dtype='float32')
        bb = paddle.arange(h, dtype='float32')
