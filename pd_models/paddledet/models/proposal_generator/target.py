...
def rpn_anchor_target(anchors,
    ...
    for i in range(batch_size):
        ...
        labels = paddle.full(shape=match_labels.shape, fill_value=-1, dtype='int32')
...
def label_box(anchors,
    ...
    if iou.shape[0] == 0 or n_gt_crowd == n_gt:
        ...
        default_matches = paddle.full(shape=(iou.shape[1], ), fill_value=0, dtype='int64')
        default_match_labels = paddle.full(shape=(iou.shape[1], ), fill_value=0, dtype='int32')
    ...
    match_labels = paddle.full(shape=matches.shape, fill_value=-1, dtype='int32')
...
def sample_bbox(matches,
    ...
    if n_gt == 0:
        # No truth, assign everything to background
        aa = paddle.ones(matches.shape, dtype='int32')
        gt_classes = aa * num_classes
        #return matches, match_labels + num_classes
    else:
        gt_classes = paddle.gather(gt_classes, matches)
        aa = paddle.ones_like(gt_classes)
        gt_classes = paddle.where(match_labels == 0, aa * num_classes, gt_classes)
        bb = paddle.ones_like(gt_classes)
        gt_classes = paddle.where(match_labels == -1, bb * -1, gt_classes)
    if is_cascade:
