...
class FusedLeakyReLU(nn.Layer):
    def __init__(self, channel, bias=True, negative_slope=0.2, scale=2**0.5):
        ...
        if bias:
            # self.bias = self.create_parameter((channel, ), default_initializer=_init)
            self.bias = self.create_parameter([channel, ], default_initializer=_init)
...
def fused_leaky_relu(input, bias=None, negative_slope=0.2, scale=2**0.5):
    if bias is not None:
        ...
        _out = F.leaky_relu(input + bias.reshape((1, bias.shape[0], *rest_dim)), negative_slope=0.2)
        out = (_out * scale)
        return out

    else:
        _out = F.leaky_relu(input, negative_slope=0.2)
        out = _out * scale
        return out
