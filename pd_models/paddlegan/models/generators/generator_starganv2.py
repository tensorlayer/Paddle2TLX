...
class AdaIN(nn.Layer):
    ...
    def forward(self, x, s):
        ...
        gamma, beta = paddle.chunk(x=h, chunks=2, axis=1)  # todo
...
class StarGANv2Mapping(nn.Layer):
    ...
    def forward(self, z, y):
        ...
        for i in range(idx.shape[0]):
            s += [
                out[idx[i].numpy().astype(np.int32).tolist()[0],
                    y[i].numpy().astype(np.int32).tolist()[0]]
            ]
        s = paddle.stack(s)
...
class StarGANv2Style(nn.Layer):
    ...
    def forward(self, x, y):
        # print(f"StarGANv2Style x.shape={x.shape}")
        # print(f"StarGANv2Style y.shape={y.shape}")
        # print(f"StarGANv2Style y={y}")
        h = self.shared(x)
