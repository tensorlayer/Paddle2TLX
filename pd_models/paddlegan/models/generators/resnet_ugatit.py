...
class ResnetUGATITGenerator(nn.Layer):
    def __init__(self,
        ...
        for i in range(n_blocks):
            DownBlock += [
                ResnetBlock(ngf * mult, use_bias=False) # , norm_layer=nn.InstanceNorm2D)
            ]
        ...
        self.adaptive_max_pool2d = nn.AdaptiveMaxPool2D(1)
...
class AdaILN(nn.Layer):
    def __init__(self, num_features, eps=1e-5):
        ...
        self.rho.set_value(paddle.full(shape=shape, fill_value=0.9))
...
class ILN(nn.Layer):
    def __init__(self, num_features, eps=1e-5):
        ...
        self.rho.set_value(paddle.full(shape=shape, fill_value=0.0))
        self.gamma.set_value(paddle.full(shape=shape, fill_value=1.0))
        self.beta.set_value(paddle.full(shape=shape, fill_value=0.0))
