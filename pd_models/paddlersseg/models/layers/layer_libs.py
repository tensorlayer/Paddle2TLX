...
class ConvBNReLU(nn.Layer):
    def __init__(self,
        ...
        b_init = None
        b_init = self.init_pd(b_init, **kwargs)
        self._conv = nn.Conv2D(in_channels=in_channels,
                               out_channels=out_channels,
                               kernel_size=kernel_size,
                               padding=padding,
                               bias_attr=b_init)  # todo
        ...
        # self._batch_norm = nn.BatchNorm2D(num_features=out_channels, data_format=data_format)
    ...
    def init_tlx(self, b_init, **kwargs):
        b_init = b_init if "bias_attr" in kwargs and kwargs["bias_attr"] is False else "constant"
        return b_init

    def forward(self, x):
        x = self._conv(x)
        # print(f"ConvBNReLU after  _conv x={x}")
        # x = self._batch_norm(x)
        x = self.batch_norm(x)
        # print(f"after _batch_norm x={x}")
        x = self._relu(x)
        # print(f"after _relu x={x}")
        return x
...
#             data_format = 'NCHW'
#         # self._batch_norm = SyncBatchNorm(out_channels, data_format=data_format)
#         self.batch_norm = SyncBatchNorm(out_channels, data_format=data_format)
#
...
class ConvBN(nn.Layer):
    def __init__(self,
        ...
        #     in_channels, out_channels, kernel_size, padding=padding, **kwargs)
        self._conv = nn.Conv2D(
            in_channels, out_channels, kernel_size, padding=padding, groups=groups, dilation=dilation)
        ...
        #     data_format = 'NCHW'
        # self._batch_norm = SyncBatchNorm(out_channels, data_format=data_format)
        # self.batch_norm = SyncBatchNorm(out_channels)
        self.batch_norm = nn.BatchNorm2D(num_features=out_channels)
