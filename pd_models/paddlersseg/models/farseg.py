...
# from ops.tlx_basic_sample import tlx_UpsamplingBilinear2d, Identity
from paddle2tlx.pd2tlx.utils import load_model_rsseg

# FARSEG_URLS = "https://bj.bcebos.com/paddleseg/dygraph/cityscapes/unet_cityscapes_1024x512_160k/model.pdparams"
...
class AsymmetricDecoder(nn.Layer):
    def __init__(self,
        ...
        for in_feature_output_stride in in_feature_output_strides:
            ...
            # print(f"num_upsample={num_upsample}")
            ...
            for idx in range(num_layers):
                ...
                blocks.append(conv_block(
                            ...
                            padding=1,
                            bias_attr=False))
                blocks.append(nn.BatchNorm2D(num_features=out_channels, momentum=0.1))
    ...
    def forward(self, feature_list):
        ...
        for idx, block in enumerate(self.blocks):
            # print(f"AsymmetricDecoder feature_list[{idx}]={feature_list[idx].shape}")
            decoder_feature = block(feature_list[idx])
            # print(f"AsymmetricDecoder decoder_feature={decoder_feature.shape}")
...
class FarSeg(nn.Layer):
    ...
    def forward(self, x):
        ...
        # print(f"feature_list[0]={feature_list[0].shape}")
        # print(f"feature_list[1]={feature_list[1].shape}")
        # print(f"feature_list[2]={feature_list[2].shape}")
        fpn_feature_list = self.fpn(feature_list)
        # print(f"fpn_feature_list[0]={fpn_feature_list[0].shape}")
        scene_feature = self.gap(feature_list[-1])
        # print(f"scene_feature[0]={scene_feature[0].shape}")
        refined_feature_list = self.fsr(scene_feature, fpn_feature_list)
        # print(f"refined_feature_list[0]={refined_feature_list[0].shape}")
...
def _farseg(pretrained=None, num_classes=5):
    ...
    if pretrained:
        ...
        # else:
        #     weight_path = download_and_decompress(FARSEG_URLS)
        # param = paddle.load(weight_path)
        # model.load_dict(param)
        model = load_model_rsseg(model, "farseg", pretrained)
    return model
