...
DEEPLAB_URLS= 'https://bj.bcebos.com/paddleseg/dygraph/cityscapes/deeplabv3p_resnet50_os8_cityscapes_1024x512_80k/model.pdparams'
...
class DeepLabV3P(nn.Layer):
    ...
    def forward(self, x):
        ...
        if self.data_format == 'channels_first' or self.data_format == 'NCHW':  # todo
            ori_shape = paddle.shape(x)[2:]
        else:
            ori_shape = paddle.shape(x)[1:3]
        data_format = "NCHW"  # todo
        ...
        return [
            F.interpolate(
                ...
                data_format=data_format) for logit in logit_list
        ]
...
class Decoder(nn.Layer):
    ...
    def forward(self, x, low_level_feat):
        ...
        if self.data_format == "channels_first" or self.data_format == "NCHW":  # todo
            ...
            axis = -1
        data_format = "NCHW"
        if self.data_format == "channels_last":  # todo
            data_format = "NHWC"
        x = F.interpolate(
            x,
            low_level_shape,
...
def _deeplabv3p(pretrained=None, num_classes=19, backbone="ResNet50_vd", in_channels=3, output_stride=8):
    from .backbones import ResNet50_vd
    # 'backbone_indices': (0, 3), 'aspp_ratios': (1, 12, 24, 36), 'aspp_out_channels': 256, 'align_corners': False}
    backbone = ResNet50_vd(in_channels=in_channels, output_stride=output_stride)
    # model = DeepLabV3P(num_classes=num_classes, backbone=backbone, backbone_indices=(0, 3),aspp_ratios=(1, 12, 24, 36))
    model = DeepLabV3P(num_classes=num_classes, backbone=backbone)
    if pretrained:
        # weight_path = download_and_decompress(DEEPLAB_URLS, "deeplabv3p")
        # param = paddle.load(weight_path)
        # # print(len([k for k in param.keys()]))
        # model.load_dict(param)
        model = load_model_rsseg(model, "deeplabv3p", DEEPLAB_URLS)
    return model
