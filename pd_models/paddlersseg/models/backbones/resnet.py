...
class ResNet(nn.Layer):
    ...
    def __init__(
        ...
        # self._norm_layer = nn.BatchNorm2D  # default
        # self._batch_norm = nn.BatchNorm2D  # 修改1
        ...
        # self.bn1 = self._norm_layer(self.inplanes)  # default
        # self.bn1 = self._batch_norm(self.inplanes)  # 修改1
        self.bn1 = nn.BatchNorm2D(self.inplanes)  # 修改2 - 建议不要批量替换, 保留修改记录
        self.relu = nn.ReLU()
...
def _resnet(arch, Block, depth, pretrained, **kwargs):
    ...
    if pretrained:
        ...
        # )
        # weight_path = download_and_decompress(
        #             model_urls[arch][0], arch
        #         )
        # param = paddle.load(weight_path)
        # model.set_dict(param)
        model = load_model_rsseg(model, arch, model_urls[arch][0])
