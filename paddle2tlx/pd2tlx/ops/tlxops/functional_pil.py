...
__all__ = ["to_tensor", "resize","pad","hflip","vflip","crop","to_grayscale"]
...
def to_tensor(pic, data_format='CHW'):
    ...
    # PIL Image
    if pic.mode == 'I':
        img = tlx.convert_to_tensor(np.array(pic, np.int32, copy=False))
    elif pic.mode == 'I;16':
        # cast and reshape not support int16
        img = tlx.convert_to_tensor(np.array(pic, np.int32, copy=False))
    elif pic.mode == 'F':
        img = tlx.convert_to_tensor(np.array(pic, np.float32, copy=False))
    elif pic.mode == '1':
        img = 255 * tlx.convert_to_tensor(np.array(pic, np.uint8, copy=False))
    else:
        img = tlx.convert_to_tensor(np.array(pic, copy=False))
    ...
    dtype = convert_dtype(img.dtype)
    if dtype == 'uint8':
        img = tlx.cast(img, np.float32) / 255.
