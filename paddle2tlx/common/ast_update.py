...
from .config import FILE_IMPORT_BACKEND
...
class AstUpdater(ast.NodeVisitor):
    ...
    def __init__(self, py_file_path, file_dependencies):
        ...
        self.is_import_paddle2tlx = True  # 是否添加import paddle2tlx
    ...
    @staticmethod
    def _get_prefix_api(api_part_name):
        prefix_api = None
        for api_name in CUSTOM_API.keys():
            if api_part_name in api_name:
                prefix_api = ".".join(api_name.split('.')[:-1])
        return prefix_api
    ...
    def _get_complete_api(self, api_part_name):
        ...
        if api_part_name in [i.split('.')[-1] for i in CUSTOM_API.keys()]:
            if api_part_name == "eval":
                return api_part_name, dep_info
            prefix_api = self._get_prefix_api(api_part_name)
            custom_api_paddle = prefix_api + "." + api_part_name
            # custom_api_tlx = CUSTOM_API[prefix_api + "." + api_part_name][0]
            return custom_api_paddle, dep_info
        for i in range(len(self.scopes_and_dependencies)):
            i = -(i + 1)
            dep_info = self.scopes_and_dependencies[i]
            if isinstance(dep_info, DepInfo):
    ...
    def _rename(self, name, dep_info, paddle_api, tlx_api):
        ...
        """
        if dep_info is None:
            if not len(tlx_api.split('.')):
                return tlx_api
            else:
                return tlx_api.split(".")[-1]
        ...
        for i, node in enumerate(self.root.body):
            if isinstance(node, ast.Import) or isinstance(node, ast.ImportFrom):
                ...
                if isinstance(node, ast.ImportFrom) and node.module in HEADER_IMPORT:
                    continue
                if self.is_import_paddle2tlx:
                    self.root.body.insert(i, ast.parse("import paddle2tlx").body[0])
                ...
                if self.py_file_path.split("\\")[-1] in FILE_IMPORT_BACKEND or \
                    self.py_file_path.strip("/")[-1] in FILE_IMPORT_BACKEND:
                    self.root.body.insert(i, ast.parse("os.environ['TL_BACKEND'] = 'paddle'").body[0])
                    self.root.body.insert(i, ast.parse("import os").body[0])
                break
    ...
    def visit_ImportFrom(self, node):
        ...
        for i, son_node in enumerate(son_nodes):
            ...
            if i == 0:
                is_remove = self.visit_alias(son_node, copy_node, node.module, node.level)  # TODO
                if not is_remove:
            ...
            if dep_info.PD_DEPENDENCY in API_MAPPER:
                ...
                for i, n in enumerate(scope_node.body):
                    if father_node == n:
                        scope_node.body[i] = ast.parse("\n").body
                is_remove = True
            elif dep_info.PD_DEPENDENCY.startswith("paddle") and not dep_info.PD_DEPENDENCY.startswith("paddle2tlx"):
    ...
    def visit_Attribute(self, node):
        ...
        if paddle_api in API_MAPPER:
            ...
            if isinstance(father_node, ast.ClassDef):
                ...
                #         return
                if paddle_api == tlx_api:
                    return None
                attr_str = self._rename(attr_str, dep_info, paddle_api, tlx_api)
                father_node.value = ast.parse(attr_str).body[0].value
                return attr_str
            elif isinstance(father_node, ast.Return):
                attr_str = self._rename(attr_str, dep_info, paddle_api, tlx_api)
                father_node.value = ast.parse(attr_str).body[0].value
                return attr_str
    ...
    def visit_arguments(self, node):
        """ 修改函数关键字参数默认api
        """
        for i, param_val in enumerate(node.defaults):
            if hasattr(param_val, "id"):  # case 1
                for k, v in REPLACE_API.items():
                    if param_val.id == k:
                        param_val.id = v
            elif hasattr(param_val, "attr"):
            ...
            elif hasattr(param_val, "func"):  # case 4 - need optimize
                try:
                    for k, v in REPLACE_API.items():
                        if param_val.func.id == k:
                            setattr(param_val, "func", ast.parse(v).body[0].value)
                    for i, keyword in enumerate(param_val.keywords):
                        if keyword.arg == "std":  # special case
                            param_val.keywords[i].arg = 'stddev'
                except Exception as err:
                    pass
            elif hasattr(param_val, "s"):  # case 5
                for j, arg in enumerate(node.args):
                    if arg.lineno == param_val.lineno:
                        for k, v in REPLACE_API.items():
                            if param_val.s == k:
                                param_val.s = v
    ...
    def visit_BinOp(self, node):
        ...
        try:
            if "(" in expr_str and ")" in expr_str:
                ...
                item_list = expr_str.replace(" ", "").split('*')
                is_all_digit = True
                ...
                if is_all_digit:
                    ...
                    father_node = self._get_father_node()
                    scope_node = self._get_scope_node()
                    expr_str_new = expr_str
                    for k, v in ATTRIBUTE_MAPPING.items():
                        if k in expr_str_new:
                            expr_str_new = expr_str_new.replace(k, v)
                    if expr_str_new != expr_str:
                        for i, n in enumerate(scope_node.body):
                            if father_node == n:
                                scope_node.body[i].value = ast.parse(expr_str_new).body[0].value
                    return expr_str
    ...
    def visit_Call(self, node):
        ...
        if isinstance(ast.parse(new_code).body[0], ast.Assign):
            ...
            # if astor.to_source(new_call_node.func).strip() == 'self.create_parameter':
            #     for item in new_call_node.keywords:
            #         if item.arg != "shape":
            #             continue
            #         if isinstance(item, ast.keyword) and isinstance(item.value, ast.Name):
            #             old_val = item.value.id
            #             item.value.id = f"({old_val}, )"
            #         elif isinstance(item.value, ast.Num):
            #             # old_val = item.value.n
            #             # item.value = ast.parse(f"({old_val}, )").body[0].value
            #             pass
            setattr(node, "func", new_call_node.func)
