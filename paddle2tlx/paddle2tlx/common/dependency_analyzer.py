...
class PdDepInfo:
    """
    Paddle依赖包信息。
    FROM代表from信息的字符串，例如：paddle；
    IMPORT代表import信息系的字符串，例如：nn.functional；
    AS代表as信息的字符串，例如：F；
    DEPENDENCY代表由FROM、IMPORT、AS三者组成的字符串，例如：from paddle import nn.functional as F。
    """
    FROM = None
    IMPORT = None
    AS = None
...
class DependencyAnalyzer(ast.NodeVisitor):
    ...
    def _get_scope_node(self):
        ...
        for i in range(len(self.scopes_and_dependencies)):
            ...
            if not isinstance(sd, PdDepInfo) and not isinstance(sd, ast.Assign):
                scope_node = sd
                break
        return scope_node
    ...
    def visit_alias(self, node, from_name=None, from_level=None):
        """ 构建PdDepInfo并将当前的PdDepInfo放入scopes_and_dependencies。
            如果import字符串为“*”，获取依赖包所在文件的依赖信息加入当前的dependencies；
            反之，直接在dependencies中加入PdDepInfo。
        """
        dep_info = PdDepInfo()
