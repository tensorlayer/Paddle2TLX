...
def flatten_tensor(inputs, channel_first=False):
    ...
    """
    if channel_first:
        input_channel_last = tensorlayerx.transpose(inputs, perm=[0, 2, 3, 1])
    else:
        input_channel_last = inputs
    output_channel_last = tensorlayerx.flatten(input_channel_last,
        start_axis=0, stop_axis=2)
...
class FCOSLoss(nn.Module):
    ...
    def __init__(self, loss_alpha=0.25, loss_gamma=2.0, iou_loss_type=\
        'giou', reg_weights=1.0):
    ...
    def __iou_loss(self, pred, targets, positive_mask, weights=None):
        ...
        ilw = tensorlayerx.minimum(plw, tlw)
        irw = tensorlayerx.minimum(prw, trw)
        ith = tensorlayerx.minimum(pth, tth)
        ibh = tensorlayerx.minimum(pbh, tbh)
        clw = tensorlayerx.maximum(plw, tlw)
        crw = tensorlayerx.maximum(prw, trw)
        cth = tensorlayerx.maximum(pth, tth)
        cbh = tensorlayerx.maximum(pbh, tbh)
        area_predict = (plw + prw) * (pth + pbh)
        area_target = (tlw + trw) * (tth + tbh)
        area_inter = (ilw + irw) * (ith + ibh)
        ious = (area_inter + 1.0) / (area_predict + area_target -
            area_inter + 1.0)
    ...
    def forward(self, cls_logits, bboxes_reg, centerness, tag_labels,
        ...
        for lvl in range(num_lvl):
            cls_logits_flatten_list.append(flatten_tensor(cls_logits[lvl],
                True))
            bboxes_reg_flatten_list.append(flatten_tensor(bboxes_reg[lvl],
                True))
            centerness_flatten_list.append(flatten_tensor(centerness[lvl],
                True))
            tag_labels_flatten_list.append(flatten_tensor(tag_labels[lvl],
                False))
            tag_bboxes_flatten_list.append(flatten_tensor(tag_bboxes[lvl],
                False))
            tag_center_flatten_list.append(flatten_tensor(tag_center[lvl],
                False))
        cls_logits_flatten = tensorlayerx.concat(cls_logits_flatten_list,
            axis=0)
        bboxes_reg_flatten = tensorlayerx.concat(bboxes_reg_flatten_list,
            axis=0)
        centerness_flatten = tensorlayerx.concat(centerness_flatten_list,
            axis=0)
        tag_labels_flatten = tensorlayerx.concat(tag_labels_flatten_list,
            axis=0)
        tag_bboxes_flatten = tensorlayerx.concat(tag_bboxes_flatten_list,
            axis=0)
        tag_center_flatten = tensorlayerx.concat(tag_center_flatten_list,
            axis=0)
        ...
        mask_positive_float = tensorlayerx.cast(mask_positive_bool, dtype=\
            'float32')
        mask_positive_float.stop_gradient = True
        num_positive_fp32 = tensorlayerx.reduce_sum(mask_positive_float)
        num_positive_fp32.stop_gradient = True
        num_positive_int32 = tensorlayerx.cast(num_positive_fp32, dtype='int32'
            )
        num_positive_int32 = num_positive_int32 * 0 + 1
        num_positive_int32.stop_gradient = True
        normalize_sum = tensorlayerx.reduce_sum(tag_center_flatten *
            mask_positive_float)
        normalize_sum.stop_gradient = True
        num_classes = cls_logits_flatten.shape[-1]
        tag_labels_flatten = tensorlayerx.ops.squeeze(tag_labels_flatten,
            axis=-1)
        tag_labels_flatten_bin = tensorlayerx.ops.OneHot(tag_labels_flatten,
            num_classes=1 + num_classes)
        tag_labels_flatten_bin = tag_labels_flatten_bin[:, 1:]
        cls_loss = paddle.nn.functional.sigmoid_focal_loss(cls_logits_flatten,
            tag_labels_flatten_bin)
        cls_loss = cls_loss / num_positive_fp32
        mask_positive_float = tensorlayerx.ops.squeeze(mask_positive_float,
            axis=-1)
        tag_center_flatten = tensorlayerx.ops.squeeze(tag_center_flatten,
            axis=-1)
        ...
        centerness_flatten = tensorlayerx.ops.squeeze(centerness_flatten,
            axis=-1)
        ctn_loss = ops.sigmoid_cross_entropy_with_logits(centerness_flatten,
            tag_center_flatten)
        ctn_loss = ctn_loss * mask_positive_float / num_positive_fp32
        loss_all = {'loss_centerness': tensorlayerx.reduce_sum(ctn_loss),
            'loss_cls': tensorlayerx.reduce_sum(cls_loss), 'loss_box':
            tensorlayerx.reduce_sum(reg_loss)}
        return loss_all
