...
def quality_focal_loss(pred, target, beta=2.0, use_sigmoid=True):
    ...
    label, score = target
    if use_sigmoid:
        func = tensorlayerx.losses.sigmoid_cross_entropy
    else:
        func = tensorlayerx.losses.binary_cross_entropy
    pred_sigmoid = tensorlayerx.ops.sigmoid(pred) if use_sigmoid else pred
    scale_factor = pred_sigmoid
    zerolabel = tensorlayerx.zeros(pred.shape, dtype='float32')
    loss = func(pred, zerolabel, reduction='none') * scale_factor.pow(beta)
    bg_class_ind = pred.shape[1]
    pos = tensorlayerx.ops.logical_and(label >= 0, label < bg_class_ind
        ).nonzero().squeeze(1)
    if pos.shape[0] == 0:
        return loss.sum(axis=1)
    pos_label = tensorlayerx.gather(label, pos, axis=0)
    pos_mask = np.zeros(pred.shape, dtype=np.int32)
    pos_mask[pos.numpy(), pos_label.numpy()] = 1
    pos_mask = tensorlayerx.convert_to_tensor(pos_mask, dtype='bool')
    score = score.unsqueeze(-1).expand([-1, pred.shape[1]]).cast('float32')
    scale_factor_new = score - pred_sigmoid
    loss_pos = func(pred, score, reduction='none') * scale_factor_new.abs(
        ).pow(beta)
    aa = tensorlayerx.ops.logical_not(pos_mask)
    loss = loss * aa + loss_pos * pos_mask
...
def distribution_focal_loss(pred, label):
    ...
    loss1 = paddle2tlx.pd2tlx.ops.tlxops.tlx_cross_entropy(pred, dis_left,
        reduction='none')
    loss2 = paddle2tlx.pd2tlx.ops.tlxops.tlx_cross_entropy(pred, dis_right,
        reduction='none')
    loss = loss1 * weight_left + loss2 * weight_right
    return loss
...
class QualityFocalLoss(nn.Module):
    ...
    def forward(self, pred, target, weight=None, avg_factor=None):
        ...
        """
        loss = self.loss_weight * quality_focal_loss(pred, target, beta=\
            self.beta, use_sigmoid=self.use_sigmoid)
