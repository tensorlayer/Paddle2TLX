...
from .bbox_head import TwoFCHead
from .bbox_head import XConvNormHead
from .roi_extractor import RoIAlign
from ..shape_spec import ShapeSpec
from ..bbox_utils import delta2bbox
from ..bbox_utils import clip_bbox
...
@register
class CascadeTwoFCHead(nn.Module):
    ...
    def __init__(self, in_channel=256, out_channel=1024, resolution=7,
        ...
        for stage in range(num_cascade_stage):
            head_per_stage = self.add_sublayer(str(stage), TwoFCHead(
                in_channel, out_channel, resolution))
...
@register
class CascadeXConvNormHead(nn.Module):
    ...
    def __init__(self, in_channel=256, num_convs=4, conv_dim=256,
        ...
        self.head_list = []
        for stage in range(num_cascade_stage):
            head_per_stage = self.add_sublayer(str(stage), XConvNormHead(
                in_channel, num_convs, conv_dim, out_channel, resolution,
                norm_type, freeze_norm, stage_name='stage{}_'.format(stage)))
            self.head_list.append(head_per_stage)
...
class CascadeHead(BBoxHead):
    ...
    def __init__(self, head, in_channel, roi_extractor=\
        ...
        num_classes=80, bbox_weight=[[10.0, 10.0, 5.0, 5.0], [20.0, 20.0, 
        10.0, 10.0], [30.0, 30.0, 15.0, 15.0]], num_cascade_stages=3,
        bbox_loss=None, reg_class_agnostic=True, stage_loss_weights=None,
        loss_normalize_pos=False, add_gt_as_proposals=[True, False, False]):
        ...
        self.stage_loss_weights = [(1.0 / num_cascade_stages) for _ in
            range(num_cascade_stages)
            ] if stage_loss_weights is None else stage_loss_weights
        self.add_gt_as_proposals = add_gt_as_proposals
        assert len(self.stage_loss_weights
        ...
        for i in range(num_cascade_stages):
            ...
            bbox_score = self.add_sublayer(score_name, nn.Linear(
                in_features=in_channel, out_features=self.num_classes + 1))
            bbox_delta = self.add_sublayer(delta_name, nn.Linear(
                in_features=in_channel, out_features=num_bbox_delta))
    ...
    def forward(self, body_feats=None, rois=None, rois_num=None, inputs=None):
        ...
        if self.training:
            rois, rois_num, targets = self.bbox_assigner(rois, rois_num,
                inputs, add_gt_as_proposals=self.add_gt_as_proposals[0])
        ...
        for i in range(self.num_cascade_stages):
            if i > 0:
                rois, rois_num = self._get_rois_from_boxes(pred_bbox,
                    inputs['im_shape'])
                if self.training:
                    rois, rois_num, targets = self.bbox_assigner(rois,
                        rois_num, inputs, i, is_cascade=True,
            ...
            if not self.reg_class_agnostic and i < self.num_cascade_stages - 1:
                ...
                if self.training:
                    deltas = deltas[tensorlayerx.ops.arange(deltas.shape[0]
                        ), labels]
                else:
                    deltas = deltas[((deltas + 10000) * F.one_hot(labels,
                        num_classes=self.num_classes).unsqueeze(-1) != 0).
                        nonzero(as_tuple=True)].reshape([deltas.shape[0], 4])
        ...
        if self.training:
            ...
            for stage, value in enumerate(zip(head_out_list, targets_list)):
                ...
                loss_stage = self.get_loss(scores, deltas, targets, rois,
                    self.bbox_weight[stage], loss_normalize_pos=self.
                    loss_normalize_pos)
                for k, v in loss_stage.items():
                    loss[k + '_stage{}'.format(stage)
                        ] = v * self.stage_loss_weights[stage]
    ...
    def _get_rois_from_boxes(self, boxes, im_shape):
        ...
        for i, boxes_per_image in enumerate(boxes):
            ...
            if self.training:
                ...
                if keep.shape[0] == 0:
                    keep = tensorlayerx.zeros([1], dtype='int32')
                clip_box = tensorlayerx.gather(clip_box, keep)
            rois.append(clip_box)
        rois_num = tensorlayerx.concat([paddle2tlx.pd2tlx.ops.tlxops.
            tlx_get_tensor_shape(r)[0] for r in rois])
        return rois, rois_num

    def _get_pred_bbox(self, deltas, proposals, weights):
        pred_proposals = tensorlayerx.concat(proposals) if len(proposals
            ) > 1 else proposals[0]
        pred_bbox = delta2bbox(deltas, pred_proposals, weights)
        pred_bbox = tensorlayerx.reshape(pred_bbox, [-1, deltas.shape[-1]])
        num_prop = []
        for p in proposals:
    ...
    def get_prediction(self, head_out_list):
        ...
        """
        pred_list = []
        scores_list = [tensorlayerx.ops.softmax(head[0]) for head in
            head_out_list]
        aa = tensorlayerx.ops.add_n(scores_list)
        scores = aa / self.num_cascade_stages
