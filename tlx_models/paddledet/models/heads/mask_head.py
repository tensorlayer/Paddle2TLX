    ...
    def __init__(self, in_channel=256, out_channel=256, num_convs=4,
        ...
        if norm_type == 'gn':
            for i in range(self.num_convs):
                ...
                mask_conv.add_sublayer(conv_name, ConvNormLayer(ch_in=\
                    in_channel if i == 0 else out_channel, ch_out=\
                    out_channel, filter_size=3, stride=1, norm_type=self.
                    norm_type, initializer=HeNormal(fan_in=fan_conv),
                    skip_quant=True))
                mask_conv.add_sublayer(conv_name + 'act', nn.ReLU())
        else:
            for i in range(self.num_convs):
                conv_name = 'mask_inter_feat_{}'.format(i + 1)
                conv = nn.GroupConv2d(in_channels=in_channel if i == 0 else
                    out_channel, out_channels=out_channel, kernel_size=3,
                    padding=1, W_init=tensorlayerx.nn.initializers.
                    xavier_uniform(), data_format='channels_first')
                conv.skip_quant = True
                mask_conv.add_sublayer(conv_name, conv)
                mask_conv.add_sublayer(conv_name + 'act', nn.ReLU())
        mask_conv.add_sublayer('conv5_mask', paddle2tlx.pd2tlx.ops.tlxops.
            tlx_ConvTranspose2d(in_channels=self.in_channel, out_channels=\
            self.out_channel, kernel_size=2, stride=2, data_format=\
            'channels_first', W_init=tensorlayerx.nn.initializers.
            xavier_uniform(), padding=0))
...
class MaskHead(nn.Module):
    ...
    def __init__(self, head, roi_extractor=_get_class_default_kwargs(
        RoIAlign), mask_assigner='MaskAssigner', num_classes=80,
        share_bbox_feat=False, export_onnx=False):
        ...
        self.bbox_head = None
        self.mask_fcn_logits = nn.GroupConv2d(in_channels=self.in_channels,
            out_channels=self.num_classes, kernel_size=1, W_init=\
            tensorlayerx.nn.initializers.xavier_uniform(), padding=0,
    ...
    def from_config(cls, cfg, input_shape):
        ...
        return {'roi_extractor': roi_pooler, 'head': head}

    def get_loss(self, mask_logits, mask_label, mask_target, mask_weight):
        mask_label = tensorlayerx.ops.OneHot(mask_label, self.num_classes
            ).unsqueeze([2, 3])
        mask_label = paddle.expand_as(mask_label, mask_logits)
        mask_label.stop_gradient = True
        mask_pred = tensorlayerx.gather_nd(mask_logits, paddle2tlx.pd2tlx.
            ops.tlxops.tlx_nonzero(mask_label))
        shape = mask_logits.shape
        mask_pred = tensorlayerx.reshape(mask_pred, [tlx_get_tensor_shape[0
            ], tlx_get_tensor_shape[2], tlx_get_tensor_shape[3]])
        mask_target = mask_target.cast('float32')
        mask_weight = mask_weight.unsqueeze([1, 2])
        loss_mask = tensorlayerx.losses.sigmoid_cross_entropy(mask_pred,
    ...
    def forward_train(self, body_feats, rois, rois_num, inputs, targets,
        ...
        rois, rois_num, tgt_classes, tgt_masks, mask_index, tgt_weights = (self
            .mask_assigner(rois, tgt_labels, tgt_gt_inds, inputs))
        if self.share_bbox_feat:
            rois_feat = tensorlayerx.gather(bbox_feat, mask_index)
        else:
    ...
    def forward_test(self, body_feats, rois, rois_num, scale_factor,
        ...
        if not self.export_onnx and rois.shape[0] == 0:
            mask_out = tensorlayerx.constant(shape=[1, 1, 1], value=-1)
        else:
            bbox = [rois[:, 2:]]
            ...
            if self.num_classes == 1:
                mask_out = tensorlayerx.ops.sigmoid(mask_logit)[:, 0, :, :]
            else:
                num_masks = paddle2tlx.pd2tlx.ops.tlxops.tlx_get_tensor_shape(
                    mask_logit)[0]
                index = tensorlayerx.ops.arange(num_masks).cast('int32')
                mask_out = mask_logit[index, labels]
                mask_out_shape = (paddle2tlx.pd2tlx.ops.tlxops.
                    tlx_get_tensor_shape(mask_out))
                mask_out = tensorlayerx.reshape(mask_out, [paddle2tlx.
                    pd2tlx.ops.tlxops.tlx_get_tensor_shape(index)[0],
                    mask_out_shape[-2], mask_out_shape[-1]])
                mask_out = tensorlayerx.ops.sigmoid(mask_out)
        return mask_out

    def forward(self, body_feats, rois, rois_num, inputs, targets=None,
        bbox_feat=None, feat_func=None):
        if self.training:
