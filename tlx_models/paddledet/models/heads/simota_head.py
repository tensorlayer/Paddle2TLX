...
class OTAHead(GFLHead):
    ...
    def __init__(self, conv_feat='FCOSFeat', dgqp_module=None, num_classes=\
        ...
        'GIoULoss', assigner='SimOTAAssigner', reg_max=16, feat_in_chan=256,
        nms=None, nms_pre=1000, cell_offset=0):
        super(OTAHead, self).__init__(conv_feat=conv_feat, dgqp_module=\
    ...
    def _get_target_single(self, flatten_cls_pred,
        flatten_center_and_stride, flatten_bbox, gt_bboxes, gt_labels):
        """Compute targets for priors in a single image.
        """
        pos_num, label, label_weight, bbox_target = self.assigner(tensorlayerx
            .ops.sigmoid(flatten_cls_pred), flatten_center_and_stride,
    ...
    def get_loss(self, head_outs, gt_meta):
        ...
        num_level_anchors = [(featmap.shape[-2] * featmap.shape[-1]) for
            featmap in cls_scores]
        num_imgs = gt_meta['im_id'].shape[0]
        featmap_sizes = [[featmap.shape[-2], featmap.shape[-1]] for featmap in
            cls_scores]
        decode_bbox_preds = []
        center_and_strides = []
        for featmap_size, stride, bbox_pred in zip(featmap_sizes, self.
            fpn_stride, bbox_preds):
            yy, xx = self.get_single_level_center_point(featmap_size,
                stride, self.cell_offset)
            center_and_stride = tensorlayerx.ops.stack([xx, yy, stride,
                stride], -1).tile([num_imgs, 1, 1])
            center_and_strides.append(center_and_stride)
            center_in_feature = center_and_stride.reshape([-1, 4])[:, :-2
                ] / stride
            bbox_pred = bbox_pred.transpose([0, 2, 3, 1]).reshape([num_imgs,
                -1, 4 * (self.reg_max + 1)])
            pred_distances = self.distribution_project(bbox_pred)
            decode_bbox_pred_wo_stride = distance2bbox(center_in_feature,
                pred_distances).reshape([num_imgs, -1, 4])
            decode_bbox_preds.append(decode_bbox_pred_wo_stride * stride)
        flatten_cls_preds = [cls_pred.transpose([0, 2, 3, 1]).reshape([
            num_imgs, -1, self.cls_out_channels]) for cls_pred in cls_scores]
        flatten_cls_preds = tensorlayerx.concat(flatten_cls_preds, axis=1)
        flatten_bboxes = tensorlayerx.concat(decode_bbox_preds, axis=1)
        flatten_center_and_strides = tensorlayerx.concat(center_and_strides,
            axis=1)
        gt_boxes, gt_labels = gt_meta['gt_bbox'], gt_meta['gt_class']
        pos_num_l, label_l, label_weight_l, bbox_target_l = [], [], [], []
        for flatten_cls_pred, flatten_center_and_stride, flatten_bbox, gt_box, gt_label in zip(
            flatten_cls_preds.detach(), flatten_center_and_strides.detach(),
            flatten_bboxes.detach(), gt_boxes, gt_labels):
            pos_num, label, label_weight, bbox_target = (self.
                _get_target_single(flatten_cls_pred,
                flatten_center_and_stride, flatten_bbox, gt_box, gt_label))
            pos_num_l.append(pos_num)
            label_l.append(label)
            label_weight_l.append(label_weight)
            bbox_target_l.append(bbox_target)
        labels = tensorlayerx.convert_to_tensor(np.stack(label_l, axis=0))
        label_weights = tensorlayerx.convert_to_tensor(np.stack(
            label_weight_l, axis=0))
        bbox_targets = tensorlayerx.convert_to_tensor(np.stack(
            bbox_target_l, axis=0))
        ...
        for cls_score, bbox_pred, center_and_strides, labels, label_weights, bbox_targets, stride in zip(
            ...
            if len(pos_inds) > 0:
                ...
                aa = tensorlayerx.gather(center_and_strides[:, :-2],
                    pos_inds, axis=0)
                pos_centers = aa / stride
...
class OTAVFLHead(OTAHead):
    ...
    def __init__(self, conv_feat='FCOSFeat', dgqp_module=None, num_classes=\
        ...
        'GIoULoss', assigner='SimOTAAssigner', reg_max=16, feat_in_chan=256,
        nms=None, nms_pre=1000, cell_offset=0):
        super(OTAVFLHead, self).__init__(conv_feat=conv_feat, dgqp_module=\
            dgqp_module, num_classes=num_classes, fpn_stride=fpn_stride,
            prior_prob=prior_prob, loss_class=loss_class, loss_dfl=loss_dfl,
            loss_bbox=loss_bbox, reg_max=reg_max, feat_in_chan=feat_in_chan,
            nms=nms, nms_pre=nms_pre, cell_offset=cell_offset)
    ...
    def get_loss(self, head_outs, gt_meta):
        ...
        for featmap_size, stride, bbox_pred in zip(featmap_sizes, self.
            ...
            yy, xx = self.get_single_level_center_point(featmap_size,
                stride, self.cell_offset)
            strides = tensorlayerx.constant(shape=len(xx), value=stride)
            center_and_stride = tensorlayerx.ops.stack([xx, yy, strides,
                strides], -1).tile([num_imgs, 1, 1])
            center_and_strides.append(center_and_stride)
            center_in_feature = center_and_stride.reshape([-1, 4])[:, :-2
                ] / stride
            bbox_pred = bbox_pred.transpose([0, 2, 3, 1]).reshape([num_imgs,
        ...
        for cls_score, bbox_pred, center_and_strides, labels, label_weights, bbox_targets, stride in zip(
            ...
            if len(pos_inds) > 0:
                ...
                bbox_iou = bbox_overlaps(pos_decode_bbox_pred.detach().
                    numpy(), pos_decode_bbox_targets.detach().numpy(),
                    is_aligned=True)
                pos_labels = tensorlayerx.gather(labels, pos_inds, axis=0)
            ...
            loss_vfl = self.loss_vfl(cls_score, vfl_score, avg_factor=\
                num_pos_avg_per_gpu)
            loss_bbox_list.append(loss_bbox)
            loss_dfl_list.append(loss_dfl)
        ...
        avg_factor = max(avg_factor.item(), 1)
        if avg_factor <= 0:
            loss_vfl = tensorlayerx.convert_to_tensor(0, dtype='float32',
                stop_gradient=False)
            loss_bbox = tensorlayerx.convert_to_tensor(0, dtype='float32',
                stop_gradient=False)
            loss_dfl = tensorlayerx.convert_to_tensor(0, dtype='float32',
