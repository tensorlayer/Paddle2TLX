...
class FCOSFeat(nn.Module):
    ...
    def __init__(self, feat_in=256, feat_out=256, num_convs=4, norm_type=\
        ...
        for i in range(self.num_convs):
            ...
            cls_conv = self.add_sublayer(cls_conv_name, ConvNormLayer(ch_in
                =in_c, ch_out=feat_out, filter_size=3, stride=1, norm_type=\
                norm_type, use_dcn=use_dcn, bias_on=True, lr_scale=2.0))
            self.cls_subnet_convs.append(cls_conv)
            reg_conv_name = 'fcos_head_reg_tower_conv_{}'.format(i)
            reg_conv = self.add_sublayer(reg_conv_name, ConvNormLayer(ch_in
                =in_c, ch_out=feat_out, filter_size=3, stride=1, norm_type=\
                norm_type, use_dcn=use_dcn, bias_on=True, lr_scale=2.0))
    ...
    def forward(self, fpn_feat):
        ...
        reg_feat = fpn_feat
        for i in range(self.num_convs):
            cls_feat = tensorlayerx.ops.relu(self.cls_subnet_convs[i](cls_feat)
                )
            reg_feat = tensorlayerx.ops.relu(self.reg_subnet_convs[i](reg_feat)
                )
...
class FCOSHead(nn.Module):
    ...
    def __init__(self, fcos_feat, num_classes=80, fpn_stride=[8, 16, 32, 64,
        128], prior_prob=0.01, fcos_loss='FCOSLoss', norm_reg_targets=True,
        ...
        self.fcos_head_cls = self.add_sublayer(conv_cls_name, nn.
            GroupConv2d(in_channels=256, out_channels=self.num_classes,
        ...
        self.fcos_head_reg = self.add_sublayer(conv_reg_name, nn.
            GroupConv2d(in_channels=256, out_channels=4, kernel_size=3,
            stride=1, padding=1, W_init=xavier_uniform(), b_init=\
            xavier_uniform(), data_format='channels_first'))
        conv_centerness_name = 'fcos_head_centerness'
        self.fcos_head_centerness = self.add_sublayer(conv_centerness_name,
            nn.GroupConv2d(in_channels=256, out_channels=1, kernel_size=3,
            stride=1, padding=1, W_init=xavier_uniform(), b_init=\
            xavier_uniform(), data_format='channels_first'))
        self.scales_regs = []
    ...
    def _compute_locations_by_level(self, fpn_stride, feature):
        ...
        """
        shape_fm = paddle2tlx.pd2tlx.ops.tlxops.tlx_get_tensor_shape(feature)
        shape_fm.stop_gradient = True
        h, w = shape_fm[2], shape_fm[3]
        shift_x = tensorlayerx.ops.arange(0, w * fpn_stride, fpn_stride)
        shift_y = tensorlayerx.ops.arange(0, h * fpn_stride, fpn_stride)
        shift_x = tensorlayerx.expand_dims(shift_x, axis=0)
        shift_y = tensorlayerx.expand_dims(shift_y, axis=1)
        ...
        shift_x = tensorlayerx.reshape(shift_x, shape=[-1])
        shift_y = tensorlayerx.reshape(shift_y, shape=[-1])
        aa = tensorlayerx.ops.stack([shift_x, shift_y], axis=-1)
        location = aa + float(fpn_stride) / 2
    ...
    def forward(self, fpn_feats, is_training):
        ...
        bboxes_reg_list = []
        centerness_list = []
        for scale_reg, fpn_stride, fpn_feat in zip(self.scales_regs, self.
            fpn_stride, fpn_feats):
            ...
            if self.norm_reg_targets:
                bbox_reg = tensorlayerx.ops.relu(bbox_reg)
                if not is_training:
                    bbox_reg = bbox_reg * fpn_stride
            else:
                bbox_reg = tensorlayerx.ops.exp(bbox_reg)
        ...
        if not is_training:
            ...
            return (locations_list, cls_logits_list, bboxes_reg_list,
                centerness_list)
        else:
