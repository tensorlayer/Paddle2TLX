...
from .target import rpn_anchor_target
from .target import generate_proposal_target
from .target import generate_mask_target
from .target import libra_generate_proposal_target
import numpy as np
...
class RPNTargetAssign(object):
    ...
    def __init__(self, batch_size_per_im=256, fg_fraction=0.5,
        positive_overlap=0.7, negative_overlap=0.3, ignore_thresh=-1.0,
        use_random=True, assign_on_cpu=False):
    ...
    def __call__(self, inputs, anchors):
        ...
        tgt_labels, tgt_bboxes, tgt_deltas = rpn_anchor_target(anchors,
            gt_boxes, self.batch_size_per_im, self.positive_overlap, self.
            negative_overlap, self.fg_fraction, self.use_random, batch_size,
            self.ignore_thresh, is_crowd, assign_on_cpu=self.assign_on_cpu)
...
class BBoxAssigner(object):
    ...
    def __init__(self, batch_size_per_im=512, fg_fraction=0.25, fg_thresh=\
        0.5, bg_thresh=0.5, ignore_thresh=-1.0, use_random=True,
        cascade_iou=[0.5, 0.6, 0.7], num_classes=80, assign_on_cpu=False):
    ...
    def __call__(self, rpn_rois, rpn_rois_num, inputs, stage=0, is_cascade=\
        False, add_gt_as_proposals=True):
        gt_classes = inputs['gt_class']
        gt_boxes = inputs['gt_bbox']
        is_crowd = inputs.get('is_crowd', None)
        outs = generate_proposal_target(rpn_rois, gt_classes, gt_boxes,
            self.batch_size_per_im, self.fg_fraction, self.fg_thresh, self.
            bg_thresh, self.num_classes, self.ignore_thresh, is_crowd, self
            .use_random, is_cascade, self.cascade_iou[stage], self.
            assign_on_cpu, add_gt_as_proposals)
        rois = outs[0]
        rois_num = outs[-1]
        targets = outs[1:4]
...
class BBoxLibraAssigner(object):
    ...
    def __init__(self, batch_size_per_im=512, fg_fraction=0.25, fg_thresh=\
        0.5, bg_thresh=0.5, use_random=True, cascade_iou=[0.5, 0.6, 0.7],
        num_classes=80, num_bins=3):
    ...
    def __call__(self, rpn_rois, rpn_rois_num, inputs, stage=0, is_cascade=\
        ...
        gt_classes = inputs['gt_class']
        gt_boxes = inputs['gt_bbox']
        outs = libra_generate_proposal_target(rpn_rois, gt_classes,
            gt_boxes, self.batch_size_per_im, self.fg_fraction, self.
            fg_thresh, self.bg_thresh, self.num_classes, self.use_random,
            is_cascade, self.cascade_iou[stage], self.num_bins)
        rois = outs[0]
        rois_num = outs[-1]
        targets = outs[1:4]
        return rois, rois_num, targets
