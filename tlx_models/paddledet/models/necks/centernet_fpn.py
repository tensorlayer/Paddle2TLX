...
from models.backbones.hardnet import ConvLayer
from models.backbones.hardnet import HarDBlock
...
class BasicConv(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, kernel_size, stride=1,
        padding=0, dilation=1, groups=1, relu=True, bn=True, bias_attr=False):
        super(BasicConv, self).__init__()
        self.out_channels = out_planes
        self.conv = nn.GroupConv2d(kernel_size=kernel_size, stride=stride,
            padding=padding, dilation=dilation, in_channels=in_planes,
            out_channels=out_planes, b_init=bias_attr, n_group=groups,
            data_format='channels_first')
        self.bn = nn.BatchNorm2d(epsilon=1e-05, momentum=0.01, num_features
            =out_planes, data_format='channels_first') if bn else None
...
class ChannelPool(nn.Module):
    ...
    def forward(self, x):
        return tensorlayerx.concat((tensorlayerx.reduce_max(x, 1).unsqueeze
            (1), tensorlayerx.reduce_mean(x, 1).unsqueeze(1)), axis=1)
...
class SpatialGate(nn.Module):
    ...
    def forward(self, x):
        ...
        scale = tensorlayerx.ops.sigmoid(x_out)
        return x * scale
...
class IDAUp(nn.Module):
    ...
    def __init__(self, ch_ins, ch_out, up_strides, dcn_v2=True):
        ...
        for i in range(1, len(ch_ins)):
            ...
            proj = nn.Sequential([ConvNormLayer(ch_in, ch_out, filter_size=\
                3, stride=1, use_dcn=dcn_v2, bias_on=dcn_v2, norm_decay=\
                None, dcn_lr_scale=1.0, dcn_regularizer=None, initializer=\
                random_uniform(-stdv, stdv)), nn.ReLU()])
            node = nn.Sequential([ConvNormLayer(ch_out, ch_out, filter_size
                =3, stride=1, use_dcn=dcn_v2, bias_on=dcn_v2, norm_decay=\
                None, dcn_lr_scale=1.0, dcn_regularizer=None, initializer=\
                random_uniform(-stdv, stdv)), nn.ReLU()])
            kernel_size = up_s * 2
            ...
            stdv = 1.0 / math.sqrt(fan_in)
            up = paddle2tlx.pd2tlx.ops.tlxops.tlx_ConvTranspose2d(in_channels
                =ch_out, out_channels=ch_out, kernel_size=up_s * 2, stride=\
                up_s, padding=up_s // 2, data_format='channels_first',
                b_init=False, n_group=ch_out)
    ...
    def forward(self, inputs, start_level, end_level):
        for i in range(start_level + 1, end_level):
            ...
            inputs[i] = node(tensorlayerx.add(inputs[i], inputs[i - 1]))
        return inputs
...
class DLAUp(nn.Module):
    ...
    def __init__(self, start_level, channels, scales, ch_in=None, dcn_v2=True):
        ...
        for i in range(len(channels) - 1):
            j = -i - 2
            setattr(self, 'ida_{}'.format(i), IDAUp(ch_in[j:], channels[j],
                scales[j:] // scales[j], dcn_v2=dcn_v2))
            scales[j + 1:] = scales[j]
...
@register
@serializable
class CenterNetDLAFPN(nn.Module):
    """
    Args:
    ...
    def __init__(self, in_channels, down_ratio=4, last_level=5, out_channel
        =0, first_level=None, dcn_v2=True, with_sge=False):
        super(CenterNetDLAFPN, self).__init__()
        self.first_level = int(np.log2(down_ratio)
            ) if first_level is None else first_level
        ...
        self.dla_up = DLAUp(self.first_level, in_channels[self.first_level:
            ], scales, dcn_v2=dcn_v2)
        self.out_channel = out_channel
        if out_channel == 0:
            self.out_channel = in_channels[self.first_level]
        self.ida_up = IDAUp(in_channels[self.first_level:self.last_level],
            self.out_channel, [(2 ** i) for i in range(self.last_level -
            self.first_level)], dcn_v2=dcn_v2)
    ...
    def forward(self, body_feats):
        ...
        if self.down_ratio != 4:
            feat = paddle.nn.functional.interpolate(feat, scale_factor=self
                .down_ratio // 4, mode='bilinear', align_corners=True)
        return feat
...
class TransitionUp(nn.Module):
    ...
    def forward(self, x, skip):
        ...
        out = paddle.nn.functional.interpolate(x, size=(w, h), mode=\
            'bilinear', align_corners=True)
        out = tensorlayerx.concat([out, skip], 1)
        return out


@register
@serializable
class CenterNetHarDNetFPN(nn.Module):
    """
    Args:
        in_channels (list): number of input feature channels from backbone.
    ...
    def __init__(self, in_channels, num_layers=85, down_ratio=4,
        first_level=None, last_level=4, out_channel=0):
        super(CenterNetHarDNetFPN, self).__init__()
        self.first_level = int(np.log2(down_ratio)
            ) - 1 if first_level is None else first_level
        assert self.first_level >= 0, 'first level in CenterNetDLAFPN should be greater or equal to 0, but received {}'.format(
            self.first_level)
        self.down_ratio = down_ratio
        self.last_level = last_level
        self.last_pool = paddle2tlx.pd2tlx.ops.tlxops.tlx_AvgPool2d(kernel_size
            =2, stride=2)
        ...
        self.avg9x9 = paddle2tlx.pd2tlx.ops.tlxops.tlx_AvgPool2d(kernel_size
            =(9, 9), stride=1, padding=(4, 4))
        ...
        for i in range(3):
            ...
            if i < self.skip_lv:
                ...
                cur_ch = prev_ch
            self.conv1x1_up.append(ConvLayer(cur_ch, ch_list2[i],
                kernel_size=1))
            cur_ch = ch_list2[i]
            cur_ch -= self.SC[i]
            cur_ch *= 3
    ...
    def forward(self, body_feats):
        ...
        x = tensorlayerx.concat([x, x2, x3], 1)
        x = self.last_blk(x)
        for i in range(3):
            ...
            x_new = tensorlayerx.concat([x_ch, x2, x3], 1)
            x = self.denseBlocksUp[i](x_new)
        scs = [x]
        for i in range(3):
            if self.SC[i] > 0:
                scs.insert(0, paddle.nn.functional.interpolate(x_sc[i],
                    size=(x.shape[2], x.shape[3]), mode='bilinear',
                    align_corners=True))
        neck_feat = tensorlayerx.concat(scs, 1)
        return neck_feat
