...
from models.post_process import nms
__all__ = ['BaseArch']
...
class BaseArch(nn.Module):
    ...
    def load_meanstd(self, cfg_transform):
        ...
        for item in cfg_transform:
            if 'NormalizeImage' in item:
                mean = np.array(item['NormalizeImage']['mean'], dtype=np.
                    float32)
        ...
        if self.data_format == 'NHWC':
            self.scale = tensorlayerx.convert_to_tensor(scale / std).reshape((
                1, 1, 1, 3))
            self.bias = tensorlayerx.convert_to_tensor(-mean / std).reshape((
                1, 1, 1, 3))
        else:
            self.scale = tensorlayerx.convert_to_tensor(scale / std).reshape((
                1, 3, 1, 1))
    ...
    def forward(self, inputs):
        if self.data_format == 'NHWC':
            image = inputs['image']
            inputs['image'] = tensorlayerx.transpose(image, [0, 2, 3, 1])
        if self.fuse_norm:
            image = inputs['image']
    ...
    def merge_multi_scale_predictions(self, outs):
        ...
        final_boxes = []
        all_scale_outs = tensorlayerx.concat([o['bbox'] for o in outs]).numpy()
        for c in range(num_classes):
            idxs = all_scale_outs[:, 0] == c
            if np.count_nonzero(idxs) == 0:
                continue
            r = nms(all_scale_outs[idxs, 1:], nms_threshold)
            final_boxes.append(np.concatenate([np.full((r.shape[0], 1), c),
                r], 1))
        out = np.concatenate(final_boxes)
        out = np.concatenate(sorted(out, key=lambda e: e[1])[-keep_top_k:]
            ).reshape((-1, 6))
        out = {'bbox': tensorlayerx.convert_to_tensor(out), 'bbox_num':
            tensorlayerx.convert_to_tensor(np.array([out.shape[0]]))}
        return out
