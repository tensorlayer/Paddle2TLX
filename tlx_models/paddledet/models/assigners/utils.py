...
import paddle2tlx
import tensorlayerx
__all__ = ['gather_topk_anchors', 'check_points_inside_bboxes',
...
def gather_topk_anchors(metrics, topk, largest=True, topk_mask=None, eps=1e-09
    ...
    topk_metrics, topk_idxs = tensorlayerx.ops.topk(metrics, topk, axis=-1,
        largest=largest)
    if topk_mask is None:
        topk_mask = (topk_metrics.max(axis=-1, keepdim=True) > eps).astype(
            metrics.dtype)
    is_in_topk = tensorlayerx.ops.OneHot(topk_idxs, num_anchors).sum(axis=-2
        ).astype(metrics.dtype)
    return is_in_topk * topk_mask


def check_points_inside_bboxes(points, bboxes, center_radius_tensor=None,
    eps=1e-09):
    """
    Args:
    ...
    r = xmax - x
    b = ymax - y
    delta_ltrb = tensorlayerx.concat([l, t, r, b], axis=-1)
    ...
    if center_radius_tensor is not None:
        ...
        delta_ltrb_c = tensorlayerx.concat([l, t, r, b], axis=-1)
        is_in_center = delta_ltrb_c.min(axis=-1) > eps
        return tensorlayerx.ops.logical_and(is_in_bboxes, is_in_center
            ), tensorlayerx.ops.logical_or(is_in_bboxes, is_in_center)
...
def compute_max_iou_anchor(ious):
    ...
    is_max_iou = tensorlayerx.ops.OneHot(max_iou_index, num_max_boxes
        ).transpose([0, 2, 1])
...
def compute_max_iou_gt(ious):
    ...
    is_max_iou = tensorlayerx.ops.OneHot(max_iou_index, num_anchors)
    return is_max_iou.astype(ious.dtype)


def generate_anchors_for_grid_cell(feats, fpn_strides, grid_cell_size=5.0,
    grid_cell_offset=0.5, dtype='float32'):
    """
    ...
    for feat, stride in zip(feats, fpn_strides):
        ...
        anchor = tensorlayerx.ops.stack([shift_x - cell_half_size, shift_y -
            cell_half_size, shift_x + cell_half_size, shift_y +
            cell_half_size], axis=-1).astype(dtype)
        anchor_point = tensorlayerx.ops.stack([shift_x, shift_y], axis=-1
            ).astype(dtype)
        anchors.append(anchor.reshape([-1, 4]))
        anchor_points.append(anchor_point.reshape([-1, 2]))
        num_anchors_list.append(len(anchors[-1]))
        stride_tensor.append(tensorlayerx.constant(shape=[num_anchors_list[
            -1], 1], dtype=dtype, value=stride))
