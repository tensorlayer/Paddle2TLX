...
from paddle.common_ops_import import check_type
__all__ = ['prior_box', 'generate_proposals', 'multiclass_nms',
    'distribute_fpn_proposals', 'batch_norm', 'mish', 'silu', 'swish',
    'identity']
...
def mish(x):
    return paddle.nn.functional.mish(x) if hasattr(F, mish) else x * F.tanh(F
        .softplus(x))
...
def batch_norm(ch, norm_type='bn', norm_decay=0.0, freeze_norm=False,
    initializer=None, data_format='channels_first'):
    norm_layer = nn.BatchNorm2d(data_format='channels_first', num_features=ch)
...
def distribute_fpn_proposals(fpn_rois, min_level, max_level, refer_level,
    ...
    **This op only takes LoDTensor as input.** In Feature Pyramid Networks 
    ...
    attrs = ('min_level', min_level, 'max_level', max_level, 'refer_level',
        refer_level, 'refer_scale', refer_scale, 'pixel_offset', pixel_offset)
    multi_rois, restore_ind, rois_num_per_level = (C_ops.
        distribute_fpn_proposals(fpn_rois, rois_num, num_lvl, num_lvl, *attrs))
...
def prior_box(input, image, min_sizes, max_sizes=None, aspect_ratios=[1.0],
    ...
    check_variable_and_dtype(input, 'input', ['uint8', 'int8', 'float32',
        'float64'], 'prior_box')
    ...
    attrs = ('min_sizes', min_sizes, 'aspect_ratios', aspect_ratios,
        'variances', variance, 'flip', flip, 'clip', clip, 'step_w', steps[
        0], 'step_h', steps[1], 'offset', offset,
...
def multiclass_nms(bboxes, scores, score_threshold, nms_top_k, keep_top_k,
    nms_threshold=0.3, normalized=True, nms_eta=1.0, background_label=-1,
    return_index=False, return_rois_num=True, rois_num=None, name=None):
    """
...
def generate_proposals(scores, bbox_deltas, im_shape, anchors, variances,
    pre_nms_top_n=6000, post_nms_top_n=1000, nms_thresh=0.5, min_size=0.1,
    eta=1.0, pixel_offset=False, return_rois_num=False, name=None):
    """
    **Generate proposal Faster-RCNN**
    ...
    rpn_rois, rpn_roi_probs, rpn_rois_num = C_ops.generate_proposals_v2(scores,
        bbox_deltas, im_shape, anchors, variances, *attrs)
    if not return_rois_num:
        rpn_rois_num = None
    return rpn_rois, rpn_roi_probs, rpn_rois_num


def sigmoid_cross_entropy_with_logits(input, label, ignore_index=-100,
    normalize=False):
    output = tensorlayerx.losses.sigmoid_cross_entropy(input, label,
        reduction='none')
    mask_tensor = tensorlayerx.cast(label != ignore_index, 'float32')
    output = tensorlayerx.ops.multiply(output, mask_tensor)
    if normalize:
        sum_valid_mask = tensorlayerx.reduce_sum(mask_tensor)
        output = output / sum_valid_mask
    return output


def get_static_shape(tensor):
    shape = paddle2tlx.pd2tlx.ops.tlxops.tlx_get_tensor_shape(tensor)
