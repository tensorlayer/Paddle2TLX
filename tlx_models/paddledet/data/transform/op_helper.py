...
def meet_emit_constraint(src_bbox, sample_bbox):
    ...
    if center_x >= sample_bbox[0] and center_x <= sample_bbox[2
        ] and center_y >= sample_bbox[1] and center_y <= sample_bbox[3]:
        return True
    return False
...
def is_overlap(object_bbox, sample_bbox):
    if object_bbox[0] >= sample_bbox[2] or object_bbox[2] <= sample_bbox[0
        ] or object_bbox[1] >= sample_bbox[3] or object_bbox[3] <= sample_bbox[
        1]:
        return False
    else:
        return True
        ...
        if bbox_area(new_bbox) > 0:
            ...
            if keypoints is not None:
                ...
                for j in range(len(sample_keypoint)):
                    ...
                    sample_keypoint[j] = (sample_keypoint[j] - sample_coord
                        ) / kp_len
...
def data_anchor_sampling(bbox_labels, image_width, image_height,
    scale_array, resize_width):
    ...
    if num_gt != 0:
        ...
        for scale_ind in range(0, len(scale_array) - 1):
            if area > scale_array[scale_ind] ** 2 and area < scale_array[
                scale_ind + 1] ** 2:
        ...
        if rand_idx_size == range_size:
            ...
            max_resize_val = min(2.0 * scale_array[rand_idx_size], 2 * math
                .sqrt(wid * hei))
        ...
        if sample_bbox_size < max(image_height, image_width):
            if wid <= sample_bbox_size:
                w_off_orig = np.random.uniform(xmin + wid -
                    sample_bbox_size, xmin)
            else:
                w_off_orig = np.random.uniform(xmin, xmin + wid -
                    sample_bbox_size)
            if hei <= sample_bbox_size:
                h_off_orig = np.random.uniform(ymin + hei -
                    sample_bbox_size, ymin)
            else:
                h_off_orig = np.random.uniform(ymin, ymin + hei -
                    sample_bbox_size)
        else:
        ...
        sampled_bbox = [w_off, h_off, w_off + float(sample_bbox_size /
            image_width), h_off + float(sample_bbox_size / image_height)]
        return sampled_bbox
    else:
        return 0
...
def jaccard_overlap(sample_bbox, object_bbox):
    if sample_bbox[0] >= object_bbox[2] or sample_bbox[2] <= object_bbox[0
        ] or sample_bbox[1] >= object_bbox[3] or sample_bbox[3] <= object_bbox[
        1]:
        return 0
...
def intersect_bbox(bbox1, bbox2):
    if bbox2[0] > bbox1[2] or bbox2[2] < bbox1[0] or bbox2[1] > bbox1[3
        ] or bbox2[3] < bbox1[1]:
        intersection_box = [0.0, 0.0, 0.0, 0.0]
    else:
        intersection_box = [max(bbox1[0], bbox2[0]), max(bbox1[1], bbox2[1]
...
def satisfy_sample_constraint(sampler, sample_bbox, gt_bboxes, satisfy_all=\
    False):
    if sampler[6] == 0 and sampler[7] == 0:
        return True
    satisfied = []
    for i in range(len(gt_bboxes)):
        object_bbox = [gt_bboxes[i][0], gt_bboxes[i][1], gt_bboxes[i][2],
            gt_bboxes[i][3]]
...
def crop_image_sampling(img, sample_bbox, image_width, image_height,
    ...
    sample_img[roi_y1:roi_y2, roi_x1:roi_x2] = img[cross_y1:cross_y2,
        cross_x1:cross_x2]
...
def is_poly(segm):
    assert isinstance(segm, (list, dict)), 'Invalid segm type: {}'.format(type
        (segm))
...
def draw_umich_gaussian(heatmap, center, radius, k=1):
    ...
    """
    diameter = 2 * radius + 1
    gaussian = gaussian2D((diameter, diameter), sigma_x=diameter / 6,
        sigma_y=diameter / 6)
