...
from .operators import Resize
from .op_helper import jaccard_overlap
from .op_helper import gaussian2D
from .op_helper import gaussian_radius
from .op_helper import draw_umich_gaussian
from .atss_assigner import ATSSAssigner
from scipy import ndimage
from models import bbox_utils
from utils.logger import setup_logger
from models.keypoint_utils import get_affine_transform
from models.keypoint_utils import affine_transform
logger = setup_logger(__name__)
__all__ = ['PadBatch', 'BatchRandomResize', 'Gt2YoloTarget',
    'Gt2FCOSTarget', 'Gt2TTFTarget', 'Gt2Solov2Target', 'Gt2GFLTarget',
    'Gt2CenterNetTarget', 'PadGT']
...
class PadBatch(BaseOperator):
    ...
    def __call__(self, samples, context=None):
        ...
        if isinstance(samples, typing.Sequence) and len(samples
            ) > 0 and isinstance(samples[0], typing.Sequence):
            inner_samples = samples[0]
        else:
            inner_samples = samples
        max_shape = np.array([data['image'].shape for data in inner_samples]
            ).max(axis=0)
        if coarsest_stride > 0:
            max_shape[1] = int(np.ceil(max_shape[1] / coarsest_stride) *
                coarsest_stride)
            max_shape[2] = int(np.ceil(max_shape[2] / coarsest_stride) *
                coarsest_stride)
        for data in inner_samples:
            im = data['image']
            im_c, im_h, im_w = im.shape[:]
            padding_im = np.zeros((im_c, max_shape[1], max_shape[2]), dtype
                =np.float32)
            ...
            if 'gt_segm' in data and data['gt_segm'] is not None:
                ...
                padding_segm = np.zeros((gt_segm.shape[0], max_shape[1],
                    max_shape[2]), dtype=np.uint8)
    ...
    def __call__(self, samples, context=None):
        ...
        for sample in samples:
            ...
            if 'gt_score' not in sample:
                sample['gt_score'] = np.ones((gt_bbox.shape[0], 1), dtype=\
                    np.float32)
            gt_score = sample['gt_score']
            for i, (mask, downsample_ratio) in enumerate(zip(self.
                anchor_masks, self.downsample_ratios)):
                grid_h = int(h / downsample_ratio)
                grid_w = int(w / downsample_ratio)
                target = np.zeros((len(mask), 6 + self.num_classes, grid_h,
                ...
                for b in range(gt_bbox.shape[0]):
                    ...
                    for an_idx in range(an_hw.shape[0]):
                        iou = jaccard_overlap([0.0, 0.0, gw, gh], [0.0, 0.0,
                            an_hw[an_idx, 0], an_hw[an_idx, 1]])
                        if iou > best_iou:
                            best_iou = iou
                            best_idx = an_idx
                    gi = int(gx * grid_w)
                    ...
                    if best_idx in mask:
                        ...
                        target[best_n, 2, gj, gi] = np.log(gw * w / self.
                            anchors[best_idx][0])
                        target[best_n, 3, gj, gi] = np.log(gh * h / self.
                            anchors[best_idx][1])
                    ...
                    if self.iou_thresh < 1:
                        for idx, mask_i in enumerate(mask):
                            if mask_i == best_idx:
                                continue
                            iou = jaccard_overlap([0.0, 0.0, gw, gh], [0.0,
                                0.0, an_hw[mask_i, 0], an_hw[mask_i, 1]])
                            if iou > self.iou_thresh and target[idx, 5, gj, gi
                                ] == 0.0:
                                target[idx, 0, gj, gi] = gx * grid_w - gi
                                target[idx, 1, gj, gi] = gy * grid_h - gj
                                target[idx, 2, gj, gi] = np.log(gw * w /
                                    self.anchors[mask_i][0])
                                target[idx, 3, gj, gi] = np.log(gh * h /
                                    self.anchors[mask_i][1])
...
class Gt2FCOSTarget(BaseOperator):
    ...
    def __init__(self, object_sizes_boundary, center_sampling_radius,
        downsample_ratios, norm_reg_targets=False):
        ...
        for i in range(len(self.object_sizes_boundary) - 1):
            object_sizes_of_interest.append([self.object_sizes_boundary[i],
    ...
    def _check_inside_boxes_limited(self, gt_bbox, xs, ys,
        ...
        """
        bboxes = np.reshape(gt_bbox, newshape=[1, gt_bbox.shape[0], gt_bbox
            .shape[1]])
        ...
        for lvl, stride in enumerate(self.downsample_ratios):
            ...
            clipped_box[beg:end, :, 0] = np.maximum(bboxes[beg:end, :, 0], 
                ct_x[beg:end, :] - stride_exp)
            clipped_box[beg:end, :, 1] = np.maximum(bboxes[beg:end, :, 1], 
                ct_y[beg:end, :] - stride_exp)
            clipped_box[beg:end, :, 2] = np.minimum(bboxes[beg:end, :, 2], 
                ct_x[beg:end, :] + stride_exp)
            clipped_box[beg:end, :, 3] = np.minimum(bboxes[beg:end, :, 3], 
                ct_y[beg:end, :] + stride_exp)
            beg = end
        ...
        for sample in samples:
            ...
            for i, num_pts in enumerate(num_points_each_level):
                object_scale_exp.append(np.tile(np.array([self.
                    object_sizes_of_interest[i]]), reps=[num_pts, 1]))
            ...
            if self.center_sampling_radius > 0:
                is_inside_box = self._check_inside_boxes_limited(bboxes, xs,
                    ys, num_points_each_level)
            else:
            ...
            lower_bound = np.tile(np.expand_dims(object_scale_exp[:, 0],
                axis=1), reps=[1, max_reg_targets.shape[1]])
            high_bound = np.tile(np.expand_dims(object_scale_exp[:, 1],
                axis=1), reps=[1, max_reg_targets.shape[1]])
            is_match_current_level = (max_reg_targets > lower_bound) & (
                max_reg_targets < high_bound)
            points2gtarea = np.tile(np.expand_dims(gt_area, axis=0), reps=[
                xs.shape[0], 1])
            ...
            ctn_targets = np.sqrt(reg_targets[:, ([0, 2])].min(axis=1) /
                reg_targets[:, ([0, 2])].max(axis=1) * (reg_targets[:, ([1,
                3])].min(axis=1) / reg_targets[:, ([1, 3])].max(axis=1))
                ).astype(np.float32)
            ctn_targets = np.reshape(ctn_targets, newshape=[ctn_targets.
                shape[0], 1])
            ...
            for lvl in range(len(self.downsample_ratios)):
                ...
                if self.norm_reg_targets:
                    sample['reg_target{}'.format(lvl)] = np.reshape(
                        reg_targets_by_level[lvl] / self.downsample_ratios[
                        lvl], newshape=[grid_h, grid_w, 4])
                else:
                    sample['reg_target{}'.format(lvl)] = np.reshape(
                        reg_targets_by_level[lvl], newshape=[grid_h, grid_w, 4]
                        )
                sample['labels{}'.format(lvl)] = np.reshape(labels_by_level
                    [lvl], newshape=[grid_h, grid_w, 1])
...
class Gt2GFLTarget(BaseOperator):
    ...
    def __init__(self, num_classes=80, downsample_ratios=[8, 16, 32, 64, 
        128], grid_cell_scale=4, cell_offset=0):
    ...
    def get_grid_cells(self, featmap_size, scale, stride, offset=0):
        ...
        y = y.flatten()
        x = x.flatten()
        grid_cells = np.stack([x - 0.5 * cell_size, y - 0.5 * cell_size, x +
            0.5 * cell_size, y + 0.5 * cell_size], axis=-1)
        return grid_cells
    ...
    def __call__(self, samples, context=None):
        ...
        for stride in self.downsample_ratios:
            featmap_size = int(math.ceil(h / stride)), int(math.ceil(w /
                stride))
            multi_level_grid_cells.append(self.get_grid_cells(featmap_size,
                self.grid_cell_scale, stride, self.cell_offset))
        mlvl_grid_cells_list = [multi_level_grid_cells for i in range(
            batch_size)]
        num_level_cells = [grid_cells.shape[0] for grid_cells in
            mlvl_grid_cells_list[0]]
        num_level_cells_list = [num_level_cells] * batch_size
        for i in range(batch_size):
            mlvl_grid_cells_list[i] = np.concatenate(mlvl_grid_cells_list[i])
        for sample, grid_cells, num_level_cells in zip(samples,
            mlvl_grid_cells_list, num_level_cells_list):
            ...
            pos_inds, neg_inds, pos_gt_bboxes, pos_assigned_gt_inds = (self
                .get_sample(assign_gt_inds, gt_bboxes))
...
class Gt2TTFTarget(BaseOperator):
    ...
    def __call__(self, samples, context=None):
        ...
        for sample in samples:
            heatmap = np.zeros((self.num_classes, feat_size, feat_size),
                dtype='float32')
            box_target = np.ones((4, feat_size, feat_size), dtype='float32'
                ) * -1
            ...
            ct_inds = np.stack([(gt_bbox[:, (0)] + gt_bbox[:, (2)]) / 2, (
                gt_bbox[:, (1)] + gt_bbox[:, (3)]) / 2], axis=1
                ) / self.down_ratio
            h_radiuses_alpha = (feat_hs / 2.0 * self.alpha).astype('int32')
            w_radiuses_alpha = (feat_ws / 2.0 * self.alpha).astype('int32')
...
class Gt2Solov2Target(BaseOperator):
    ...
    def __init__(self, num_grids=[40, 36, 24, 16, 12], scale_ranges=[[1, 96
        ], [48, 192], [96, 384], [192, 768], [384, 2048]], coord_sigma=0.2,
        sampling_ratio=4.0):
    ...
    def _scale_size(self, im, scale):
        ...
        resized_img = cv2.resize(im, None, None, fx=scale, fy=scale,
    ...
    def __call__(self, samples, context=None):
        ...
        for sample in samples:
            ...
            mask_feat_size = [int(im_h / self.sampling_ratio), int(im_w /
                self.sampling_ratio)]
            gt_areas = np.sqrt((gt_bboxes_raw[:, (2)] - gt_bboxes_raw[:, (0
                )]) * (gt_bboxes_raw[:, (3)] - gt_bboxes_raw[:, (1)]))
            ins_ind_label_list = []
            idx = 0
            for (lower_bound, upper_bound), num_grid in zip(self.
                scale_ranges, self.num_grids):
                hit_indices = ((gt_areas >= lower_bound) & (gt_areas <=\
                    upper_bound)).nonzero()[0]
                ...
                if num_ins == 0:
                    ins_label = np.zeros([1, mask_feat_size[0],
                        mask_feat_size[1]], dtype=np.uint8)
                ...
                for seg_mask, gt_label, half_h, half_w in zip(gt_masks,
                    ...
                    if seg_mask.sum() == 0:
                        continue
                    upsampled_size = mask_feat_size[0] * 4, mask_feat_size[1
                        ] * 4
                    center_h, center_w = ndimage.measurements.center_of_mass(
                        seg_mask)
                    coord_w = int(center_w / upsampled_size[1] // (1.0 /
                        num_grid))
                    coord_h = int(center_h / upsampled_size[0] // (1.0 /
                        num_grid))
                    top_box = max(0, int((center_h - half_h) /
                        upsampled_size[0] // (1.0 / num_grid)))
                    down_box = min(num_grid - 1, int((center_h + half_h) /
                        upsampled_size[0] // (1.0 / num_grid)))
                    left_box = max(0, int((center_w - half_w) /
                    ...
                    seg_mask = self._scale_size(seg_mask, scale=1.0 / self.
                        sampling_ratio)
                    for i in range(top, down + 1):
                        for j in range(left, right + 1):
                            label = int(i * num_grid + j)
                            cur_ins_label = np.zeros([mask_feat_size[0],
                                mask_feat_size[1]], dtype=np.uint8)
                            cur_ins_label[:seg_mask.shape[0], :seg_mask.
                                shape[1]] = seg_mask
                            ins_label.append(cur_ins_label)
                            ins_ind_label[label] = True
                            grid_order.append(sample_id * num_grid *
                                num_grid + label)
                if ins_label == []:
                    ins_label = np.zeros([1, mask_feat_size[0],
                        mask_feat_size[1]], dtype=np.uint8)
                ...
                max_ins_num[idx] = max(max_ins_num[idx], sample[
                    'ins_label{}'.format(idx)].shape[0])
                idx += 1
            ins_ind_labels = np.concatenate([ins_ind_labels_level_img for
                ins_ind_labels_level_img in ins_ind_label_list])
        ...
        for data in samples:
            for idx in range(len(self.num_grids)):
                gt_ins_data = np.zeros([max_ins_num[idx], data[
                    'ins_label{}'.format(idx)].shape[1], data['ins_label{}'
                    .format(idx)].shape[2]], dtype=np.uint8)
                gt_ins_data[0:data['ins_label{}'.format(idx)].shape[0], :, :
                    ] = data['ins_label{}'.format(idx)]
                gt_grid_order = np.zeros([max_ins_num[idx]], dtype=np.int32)
                gt_grid_order[0:data['grid_order{}'.format(idx)].shape[0]
                    ] = data['grid_order{}'.format(idx)]
...
class Gt2CenterNetTarget(BaseOperator):
    ...
    def __call__(self, sample, context=None):
        ...
        cat_spec_wh = np.zeros((self.max_objs, num_classes * 2), dtype=np.
            float32)
        cat_spec_mask = np.zeros((self.max_objs, num_classes * 2), dtype=np
            .int32)
        ...
        for i, (bbox, cls) in enumerate(zip(gt_bbox, gt_class)):
            ...
            if h > 0 and w > 0:
                ...
                ct = np.array([(bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) /
                    2], dtype=np.float32)
                ...
                gt_det.append([ct[0] - w / 2, ct[1] - h / 2, ct[0] + w / 2,
                    ct[1] + h / 2, 1, cls])
        sample.pop('gt_bbox', None)
        sample.pop('gt_class', None)
    ...
    def __call__(self, samples, context=None):
        ...
        for sample in samples:
            if self.return_gt_mask:
                sample['pad_gt_mask'] = np.zeros((num_max_boxes, 1), dtype=\
                    np.float32)
            if num_max_boxes == 0:
                continue
