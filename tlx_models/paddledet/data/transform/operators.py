...
from .op_helper import filter_and_process
from .op_helper import generate_sample_bbox
from .op_helper import clip_bbox
from .op_helper import data_anchor_sampling
from .op_helper import satisfy_sample_constraint_coverage
from .op_helper import crop_image_sampling
from .op_helper import generate_sample_bbox_square
from .op_helper import bbox_area_sampling
from .op_helper import is_poly
from .op_helper import get_border
from utils.logger import setup_logger
from models.keypoint_utils import get_affine_transform
from models.keypoint_utils import affine_transform
...
class Decode(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if 'h' not in sample:
            ...
            logger.warning(
                "The actual image height: {} is not equal to the height: {} in annotation, and update sample['h'] by actual image height."
                .format(im.shape[0], sample['h']))
            sample['h'] = im.shape[0]
        if 'w' not in sample:
            sample['w'] = im.shape[1]
        elif sample['w'] != im.shape[1]:
            logger.warning(
                "The actual image width: {} is not equal to the width: {} in annotation, and update sample['w'] by actual image width."
                .format(im.shape[1], sample['w']))
            sample['w'] = im.shape[1]
...
class DecodeCache(BaseOperator):
    ...
    def apply(self, sample, context=None):
        if self.use_cache and os.path.exists(self.cache_path(self.
            cache_root, sample['im_file'])):
            ...
            if 'keep_ori_im' in sample and sample['keep_ori_im']:
                sample['ori_image'] = im
            im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
            if self.use_cache and not os.path.exists(self.cache_path(self.
                cache_root, sample['im_file'])):
                path = self.cache_path(self.cache_root, sample['im_file'])
                self.dump(im, path)
        sample['image'] = im
...
class SniperDecodeCrop(BaseOperator):
    ...
    def __call__(self, sample, context=None):
        ...
        im = im[max(y1, 0):min(y2, im.shape[0]), max(x1, 0):min(x2, im.
            shape[1]), :]
        sample['image'] = im
        h = im.shape[0]
        w = im.shape[1]
...
class RandomErasingImage(BaseOperator):
    ...
    def apply(self, sample):
        ...
        for idx in range(gt_bbox.shape[0]):
            ...
            aspect_ratio = random.uniform(self.aspect_ratio, 1 / self.
                aspect_ratio)
...
class NormalizeImage(BaseOperator):
    ...
    def __init__(self, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225
        ...
        if not (isinstance(self.mean, list) and isinstance(self.std, list) and
            isinstance(self.is_scale, bool) and self.norm_type in [
            'mean_std', 'none']):
...
class GridMask(BaseOperator):
    ...
    def __init__(self, use_h=True, use_w=True, rotate=1, offset=False,
        ratio=0.5, mode=1, prob=0.7, upper_iter=360000):
        """
        ...
        self.gridmask_op = Gridmask(use_h, use_w, rotate=rotate, offset=\
            offset, ratio=ratio, mode=mode, prob=prob, upper_iter=upper_iter)
...
class RandomDistort(BaseOperator):
    ...
    def __init__(self, hue=[-18, 18, 0.5], saturation=[0.5, 1.5, 0.5],
        contrast=[0.5, 1.5, 0.5], brightness=[0.5, 1.5, 0.5], random_apply=\
        True, count=4, random_channel=False):
    ...
    def apply_hue(self, img):
        ...
        tyiq = np.array([[0.299, 0.587, 0.114], [0.596, -0.274, -0.321], [
            0.211, -0.523, 0.311]])
        ityiq = np.array([[1.0, 0.956, 0.621], [1.0, -0.272, -0.647], [1.0,
            -1.107, 1.705]])
    ...
    def apply(self, sample, context=None):
        ...
        if self.random_apply:
            functions = [self.apply_brightness, self.apply_contrast, self.
                apply_saturation, self.apply_hue]
    ...
    def apply(self, sample, context=None):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ...
            if 'gt_keypoint' in sample and len(sample['gt_keypoint']) > 0:
                sample['gt_keypoint'] = self.apply_keypoint(sample[
                    'gt_keypoint'], width)
...
class Resize(BaseOperator):
    ...
    def apply_image(self, image, scale):
        ...
        return cv2.resize(image, None, None, fx=im_scale_x, fy=im_scale_y,
    ...
    def apply_segm(self, segms, im_size, scale):
        ...
        def _resize_rle(rle, im_h, im_w, im_scale_x, im_scale_y):
            ...
            mask = mask_util.decode(rle)
            mask = cv2.resize(mask, None, None, fx=im_scale_x, fy=\
                im_scale_y, interpolation=self.interp)
            rle = mask_util.encode(np.array(mask, order='F', dtype=np.uint8))
            return rle
        im_h, im_w = im_size
        im_scale_x, im_scale_y = scale
        ...
        for segm in segms:
            if is_poly(segm):
                resized_segms.append([_resize_poly(poly, im_scale_x,
                    im_scale_y) for poly in segm])
            else:
                import pycocotools.mask as mask_util
                resized_segms.append(_resize_rle(segm, im_h, im_w,
    ...
    def apply(self, sample, context=None):
        ...
        if self.keep_ratio:
            ...
            im_scale = min(target_size_min / im_size_min, target_size_max /
                im_size_max)
        ...
        if 'scale_factor' in sample:
            ...
            sample['scale_factor'] = np.asarray([scale_factor[0] *
                im_scale_y, scale_factor[1] * im_scale_x], dtype=np.float32)
        else:
            sample['scale_factor'] = np.asarray([im_scale_y, im_scale_x],
                dtype=np.float32)
        if 'gt_bbox' in sample and len(sample['gt_bbox']) > 0:
            sample['gt_bbox'] = self.apply_bbox(sample['gt_bbox'], [
                im_scale_x, im_scale_y], [resize_w, resize_h])
        if 'gt_poly' in sample and len(sample['gt_poly']) > 0:
            sample['gt_poly'] = self.apply_segm(sample['gt_poly'], im_shape
                [:2], [im_scale_x, im_scale_y])
        if 'semantic' in sample and sample['semantic']:
            semantic = sample['semantic']
            semantic = cv2.resize(semantic.astype('float32'), None, None,
                fx=im_scale_x, fy=im_scale_y, interpolation=self.interp)
            semantic = np.asarray(semantic).astype('int32')
            semantic = np.expand_dims(semantic, 0)
            sample['semantic'] = semantic
        if 'gt_segm' in sample and len(sample['gt_segm']) > 0:
            masks = [cv2.resize(gt_segm, None, None, fx=im_scale_x, fy=\
                im_scale_y, interpolation=cv2.INTER_NEAREST) for gt_segm in
                sample['gt_segm']]
...
class MultiscaleTestResize(BaseOperator):
    ...
    def __init__(self, origin_target_size=[800, 1333], target_size=[],
        interp=cv2.INTER_LINEAR, use_flip=True):
        """
    ...
    def apply(self, sample, context=None):
        ...
        """
        samples = []
        resizer = Resize(self.origin_target_size, keep_ratio=True, interp=\
            self.interp)
...
class RandomResize(BaseOperator):
    ...
    def __init__(self, target_size, keep_ratio=True, interp=cv2.
        INTER_LINEAR, random_size=True, random_interp=False):
        """
        ...
        self.keep_ratio = keep_ratio
        self.interp = interp
        self.interps = [cv2.INTER_NEAREST, cv2.INTER_LINEAR, cv2.INTER_AREA,
...
class RandomExpand(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        pad = Pad(size, pad_mode=-1, offsets=offsets, fill_value=self.
            fill_value)
...
class CropWithSampling(BaseOperator):
    ...
    def apply(self, sample, context):
        ...
        while sampled_bbox:
            ...
            crop_bbox, crop_class, crop_score = filter_and_process(sample_bbox,
                gt_bbox, gt_class, scores=gt_score)
...
class CropWithDataAchorSampling(BaseOperator):
    ...
    def __init__(self, batch_sampler, anchor_sampler=None, target_size=None,
        das_anchor_scales=[16, 32, 64, 128], sampling_prob=0.5, min_size=\
        8.0, avoid_no_bbox=True):
        """
        Args:
        ...
        if prob > self.sampling_prob:
            ...
            for sampler in self.anchor_sampler:
                ...
                for i in range(sampler[1]):
                    if found >= sampler[0]:
                        break
                    sample_bbox = data_anchor_sampling(gt_bbox, image_width,
                        image_height, self.das_anchor_scales, self.target_size)
                    if sample_bbox == 0:
                        break
                    if satisfy_sample_constraint_coverage(sampler,
                        sample_bbox, gt_bbox):
                        sampled_bbox.append(sample_bbox)
                        found = found + 1
            im = np.array(im)
            while sampled_bbox:
                ...
                if 'gt_keypoint' in sample.keys():
                    ...
                    crop_bbox, crop_class, crop_score, gt_keypoints = (
                        ...
                        scores=gt_score, keypoints=keypoints))
                else:
            ...
            for sampler in self.batch_sampler:
                ...
                for i in range(sampler[1]):
                    if found >= sampler[0]:
                        break
                    sample_bbox = generate_sample_bbox_square(sampler,
...
class RandomCrop(BaseOperator):
    ...
    def __init__(self, aspect_ratio=[0.5, 2.0], thresholds=[0.0, 0.1, 0.3, 
        0.5, 0.7, 0.9], scaling=[0.3, 1.0], num_attempts=50, allow_no_crop=\
        True, cover_all_box=False, is_mask_crop=False):
                    ...
                    if isinstance(inter, (MultiPolygon, GeometryCollection)):
                        for part in inter:
                            if not isinstance(part, Polygon):
                                continue
                            part = np.squeeze(np.array(part.exterior.coords
                                [:-1]).reshape(1, -1))
                            part[0::2] -= xmin
                            part[1::2] -= ymin
                            crop_segm.append(part.tolist())
                    elif isinstance(inter, Polygon):
                        crop_poly = np.squeeze(np.array(inter.exterior.
                            coords[:-1]).reshape(1, -1))
                        crop_poly[0::2] -= xmin
                        crop_poly[1::2] -= ymin
    ...
    def apply(self, sample, context=None):
        ...
        for thresh in thresholds:
            ...
            for i in range(self.num_attempts):
                ...
                iou = self._iou_matrix(gt_bbox, np.array([crop_box], dtype=\
                    np.float32))
                if iou.max() < thresh:
                    continue
            ...
            if found:
                if self.is_mask_crop and 'gt_poly' in sample and len(sample
                    ['gt_poly']) > 0:
                    crop_polys = self.crop_segms(sample['gt_poly'],
                        valid_ids, np.array(crop_box, dtype=np.int64), h, w)
                    if [] in crop_polys:
                        delete_id = list()
                        valid_polys = list()
                ...
                if 'gt_segm' in sample:
                    sample['gt_segm'] = self._crop_segm(sample['gt_segm'],
                        crop_box)
                    sample['gt_segm'] = np.take(sample['gt_segm'],
                        valid_ids, axis=0)
                sample['image'] = self._crop_image(sample['image'], crop_box)
                sample['gt_bbox'] = np.take(cropped_box, valid_ids, axis=0)
                sample['gt_class'] = np.take(sample['gt_class'], valid_ids,
                    axis=0)
                if 'gt_score' in sample:
                    sample['gt_score'] = np.take(sample['gt_score'],
                        valid_ids, axis=0)
                if 'is_crowd' in sample:
                    sample['is_crowd'] = np.take(sample['is_crowd'],
                        valid_ids, axis=0)
                if 'difficult' in sample:
                    sample['difficult'] = np.take(sample['difficult'],
                        valid_ids, axis=0)
                return sample
        return sample
    ...
    def _crop_box_with_center_constraint(self, box, crop):
        ...
        valid = np.logical_and(crop[:2] <= centers, centers < crop[2:]).all(
            axis=1)
        valid = np.logical_and(valid, (cropped_box[:, :2] < cropped_box[:, 
            2:]).all(axis=1))
...
class RandomScaledCrop(BaseOperator):
    ...
    def __init__(self, target_dim=512, scale_range=[0.1, 2.0], interp=cv2.
        INTER_LINEAR):
    ...
    def apply(self, sample, context=None):
        ...
        resize_w = w * scale
        resize_h = h * scale
        offset_x = int(max(0, np.random.uniform(0.0, resize_w - dim)))
        offset_y = int(max(0, np.random.uniform(0.0, resize_h - dim)))
        img = cv2.resize(img, (resize_w, resize_h), interpolation=self.interp)
        img = np.array(img)
        canvas = np.zeros((dim, dim, 3), dtype=img.dtype)
        canvas[:min(dim, resize_h), :min(dim, resize_w), :] = img[offset_y:
            offset_y + dim, offset_x:offset_x + dim, :]
        sample['image'] = canvas
        sample['im_shape'] = np.asarray([resize_h, resize_w], dtype=np.float32)
        scale_factor = sample['sacle_factor']
        sample['scale_factor'] = np.asarray([scale_factor[0] * scale, 
            scale_factor[1] * scale], dtype=np.float32)
        if 'gt_bbox' in sample and len(sample['gt_bbox']) > 0:
...
class Cutmix(BaseOperator):
    ...
    def __call__(self, sample, context=None):
        ...
        gt_score = np.concatenate((gt_score1 * factor, gt_score2 * (1.0 -
            factor)), axis=0)
        ...
        if 'difficult' in sample[0]:
            ...
            is_difficult = np.concatenate((is_difficult1, is_difficult2),
                axis=0)
...
class Mixup(BaseOperator):
    ...
    def apply_image(self, img1, img2, factor):
        ...
        img[:img1.shape[0], :img1.shape[1], :] = img1.astype('float32'
            ) * factor
...
class DebugVisibleImage(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        for i in range(gt_bbox.shape[0]):
            ...
            draw.line([(xmin, ymin), (xmin, ymax), (xmax, ymax), (xmax,
                ymin), (xmin, ymin)], width=2, fill='green')
        ...
        if 'gt_keypoint' in sample.keys():
            ...
            for i in range(gt_keypoint.shape[0]):
                ...
                for j in range(int(keypoint.shape[0] / 2)):
                    ...
                    draw.ellipse((x1, y1, x1 + 5, y1 + 5), fill='green',
                        outline='green')
...
class Pad(BaseOperator):
    ...
    def apply_segm(self, segms, offsets, im_size, size):
        ...
        def _expand_rle(rle, x, y, height, width, h, w):
            ...
            rle = mask_util.encode(np.array(expanded_mask, order='F', dtype
                =np.uint8))
            return rle
        x, y = offsets
        ...
        for segm in segms:
            if is_poly(segm):
                expanded_segms.append([_expand_poly(poly, x, y) for poly in
                    segm])
            else:
                import pycocotools.mask as mask_util
                expanded_segms.append(_expand_rle(segm, x, y, height, width,
                    h, w))
...
class Poly2Mask(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        im_h = sample['h']
        im_w = sample['w']
        masks = [self._poly2mask(gt_poly, im_h, im_w) for gt_poly in sample
            ['gt_poly']]
        sample['gt_segm'] = np.asarray(masks).astype(np.uint8)
        return sample
...
class AugmentHSV(BaseOperator):
    ...
    def __init__(self, fraction=0.5, is_bgr=True, hgain=None, sgain=None,
        vgain=None):
    ...
    def apply(self, sample, context=None):
        ...
        if self.use_hsvgain:
            hsv_augs = np.random.uniform(-1, 1, 3) * [self.hgain, self.
                sgain, self.vgain]
...
class RandomResizeCrop(BaseOperator):
    ...
    def __init__(self, resizes, cropsizes, prob=0.5, mode='short',
        keep_ratio=True, interp=cv2.INTER_LINEAR, num_attempts=3,
        cover_all_box=False, allow_no_crop=False, thresholds=[0.3, 0.5, 0.7
        ], is_mask_crop=False):
        ...
        self.croper = RandomCrop(num_attempts=num_attempts, cover_all_box=\
            cover_all_box, thresholds=thresholds, allow_no_crop=\
            allow_no_crop, is_mask_crop=is_mask_crop)
    ...
    def apply(self, sample, context=None):
        if random.random() < self.prob:
            ...
            sample = self._resize(self.resizer, sample, size=_resize, mode=\
                self.mode, context=context)
            sample = self._random_crop(self.croper, sample, size=_cropsize,
                context=context)
        return sample
...
class RandomShortSideResize(BaseOperator):
    ...
    def __init__(self, short_side_sizes, max_size=None, interp=cv2.
        INTER_LINEAR, random_interp=False):
        """
    ...
    def resize(self, sample, target_size, max_size=None, interp=cv2.
        INTER_LINEAR):
        im = sample['image']
        if not isinstance(im, np.ndarray):
            raise TypeError('{}: image type is not numpy.'.format(self))
        ...
        im_scale_y, im_scale_x = target_size[1] / im.shape[0], target_size[0
            ] / im.shape[1]
        ...
        if 'gt_bbox' in sample and len(sample['gt_bbox']) > 0:
            sample['gt_bbox'] = self.apply_bbox(sample['gt_bbox'], [
                im_scale_x, im_scale_y], target_size)
        if 'gt_poly' in sample and len(sample['gt_poly']) > 0:
            sample['gt_poly'] = self.apply_segm(sample['gt_poly'], im.shape
                [:2], [im_scale_x, im_scale_y])
        if 'semantic' in sample and sample['semantic']:
            semantic = sample['semantic']
            semantic = cv2.resize(semantic.astype('float32'), target_size,
                interpolation=self.interp)
            semantic = np.asarray(semantic).astype('int32')
            semantic = np.expand_dims(semantic, 0)
            sample['semantic'] = semantic
        if 'gt_segm' in sample and len(sample['gt_segm']) > 0:
            masks = [cv2.resize(gt_segm, target_size, interpolation=cv2.
                INTER_NEAREST) for gt_segm in sample['gt_segm']]
            sample['gt_segm'] = np.asarray(masks).astype(np.uint8)
        return sample
    ...
    def apply(self, sample, context=None):
        ...
        interp = random.choice(self.interps
            ) if self.random_interp else self.interp
...
class RandomSizeCrop(BaseOperator):
    ...
    def __init__(self, min_size, max_size):
        ...
        self.min_size = min_size
        self.max_size = max_size
        from tensorlayerx.vision.transforms import Crop as paddle_crop
    ...
    def crop(self, sample, region):
        ...
        if 'gt_bbox' in sample and len(sample['gt_bbox']) > 0:
            ...
            sample['gt_bbox'] = sample['gt_bbox'][keep_index] if len(keep_index
                ) > 0 else np.zeros([0, 4], dtype=np.float32)
            sample['gt_class'] = sample['gt_class'][keep_index] if len(
                keep_index) > 0 else np.zeros([0, 1], dtype=np.float32)
            if 'gt_score' in sample:
    ...
    def apply(self, sample, context=None):
        h = random.randint(self.min_size, min(sample['image'].shape[0],
            self.max_size))
        w = random.randint(self.min_size, min(sample['image'].shape[1],
            self.max_size))
...
class WarpAffine(BaseOperator):
    ...
    def __init__(self, keep_res=False, pad=31, input_h=512, input_w=512,
        scale=0.4, shift=0.1):
        """WarpAffine
    ...
    def apply(self, sample, context=None):
        ...
        inp = cv2.warpAffine(img, trans_input, (input_w, input_h), flags=\
            cv2.INTER_LINEAR)
        sample['image'] = inp
        return sample
...
class FlipWarpAffine(BaseOperator):
    ...
    def __init__(self, keep_res=False, pad=31, input_h=512, input_w=512,
        not_rand_crop=False, scale=0.4, shift=0.1, flip=0.5, is_scale=True,
        use_random=True):
        """FlipWarpAffine
        1. Random Crop
...
class CenterRandColor(BaseOperator):
    ...
    def apply_saturation(self, img, img_gray):
        alpha = 1.0 + np.random.uniform(low=-self.saturation, high=self.
            saturation)
    ...
    def apply_brightness(self, img, img_gray):
        alpha = 1 + np.random.uniform(low=-self.brightness, high=self.
            brightness)
        img *= alpha
        return img
...
class Mosaic(BaseOperator):
    ...
    def __init__(self, prob=1.0, input_dim=[640, 640], degrees=[-10, 10],
        translate=[-0.1, 0.1], scale=[0.1, 2], shear=[-2, 2], enable_mixup=\
        True, mixup_prob=1.0, mixup_scale=[0.5, 1.5], remove_outside_box=False
        ):
    ...
    def get_mosaic_coords(self, mosaic_idx, xc, yc, w, h, input_h, input_w):
        if mosaic_idx == 0:
            ...
            x1, y1, x2, y2 = xc, yc, min(xc + w, input_w * 2), min(input_h *
                2, yc + h)
            small_coords = 0, 0, min(w, x2 - x1), min(y2 - y1, h)
        return (x1, y1, x2, y2), small_coords

    def random_affine_augment(self, img, labels=[], input_dim=[640, 640],
        degrees=[-10, 10], scales=[0.1, 2], shears=[-2, 2], translates=[-
        0.1, 0.1]):
        ...
        M[0, 2] = translation_x
        M[1, 2] = translation_y
        img = cv2.warpAffine(img, M, dsize=tuple(input_dim), borderValue=(
            114, 114, 114))
        num_gts = len(labels)
        if num_gts > 0:
    ...
    def __call__(self, sample, context=None):
        ...
        for mosaic_idx, sp in enumerate(sample[:4]):
            ...
            (l_x1, l_y1, l_x2, l_y2), (s_x1, s_y1, s_x2, s_y2
                ) = self.get_mosaic_coords(mosaic_idx, xc, yc, w, h,
                input_h, input_w)
        ...
        if len(mosaic_gt_bbox):
            ...
            if mosaic_is_crowd:
                ...
                mosaic_labels = np.concatenate([mosaic_gt_bbox,
                    mosaic_gt_class.astype(mosaic_gt_bbox.dtype),
                    mosaic_difficult.astype(mosaic_gt_bbox.dtype)], 1)
            else:
                mosaic_labels = np.concatenate([mosaic_gt_bbox,
            ...
            if self.remove_outside_box:
                ...
                mosaic_labels[:, 0] = np.clip(mosaic_labels[:, 0], 0, 2 *
                    input_w)
                mosaic_labels[:, 1] = np.clip(mosaic_labels[:, 1], 0, 2 *
                    input_h)
                mosaic_labels[:, 2] = np.clip(mosaic_labels[:, 2], 0, 2 *
                    input_w)
                mosaic_labels[:, 3] = np.clip(mosaic_labels[:, 3], 0, 2 *
                    input_h)
        else:
            mosaic_labels = np.zeros((1, 6))
        mosaic_img, mosaic_labels = self.random_affine_augment(mosaic_img,
            mosaic_labels, input_dim=self.input_dim, degrees=self.degrees,
            translates=self.translate, scales=self.scale, shears=self.shear)
        if self.enable_mixup and not len(mosaic_labels) == 0 and random.random(
            ) < self.mixup_prob:
            sample_mixup = sample[4]
            mixup_img = sample_mixup['image']
            if 'is_crowd' in sample_mixup:
                cp_labels = np.concatenate([sample_mixup['gt_bbox'],
                ...
                cp_labels = np.concatenate([sample_mixup['gt_bbox'],
                    sample_mixup['gt_class'].astype(mosaic_labels.dtype),
                    sample_mixup['difficult'].astype(mosaic_labels.dtype)], 1)
            else:
                cp_labels = np.concatenate([sample_mixup['gt_bbox'],
                    sample_mixup['gt_class'].astype(mosaic_labels.dtype)], 1)
            mosaic_img, mosaic_labels = self.mixup_augment(mosaic_img,
                mosaic_labels, self.input_dim, cp_labels, mixup_img)
        sample0 = sample[0]
    ...
    def mixup_augment(self, origin_img, origin_labels, input_dim, cp_labels,
        ...
        if len(img.shape) == 3:
            cp_img = np.ones((input_dim[0], input_dim[1], 3), dtype=np.uint8
                ) * 114
        else:
            cp_img = np.ones(input_dim, dtype=np.uint8) * 114
        cp_scale_ratio = min(input_dim[0] / img.shape[0], input_dim[1] /
            img.shape[1])
        resized_img = cv2.resize(img, (int(img.shape[1] * cp_scale_ratio),
            int(img.shape[0] * cp_scale_ratio)), interpolation=cv2.INTER_LINEAR
            )
        cp_img[:int(img.shape[0] * cp_scale_ratio), :int(img.shape[1] *
            cp_scale_ratio)] = resized_img
        cp_img = cv2.resize(cp_img, (int(cp_img.shape[1] * jit_factor), int
            (cp_img.shape[0] * jit_factor)))
        cp_scale_ratio *= jit_factor
        if FLIP:
            cp_img = cp_img[:, ::-1, :]
        origin_h, origin_w = cp_img.shape[:2]
        target_h, target_w = origin_img.shape[:2]
        padded_img = np.zeros((max(origin_h, target_h), max(origin_w,
            target_w), 3), dtype=np.uint8)
        ...
        if FLIP:
            cp_bboxes_origin_np[:, 0::2] = origin_w - cp_bboxes_origin_np[:,
                0::2][:, ::-1]
        ...
        origin_img = 0.5 * origin_img + 0.5 * padded_cropped_img.astype(np.
            float32)
...
class PadResize(BaseOperator):
    ...
    def _pad(self, img):
        ...
        if h == self.target_size[0] and w == self.target_size[1]:
            return img
        padded_img = np.full((self.target_size[0], self.target_size[1], 3),
