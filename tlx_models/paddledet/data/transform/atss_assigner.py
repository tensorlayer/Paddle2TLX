...
def bbox_overlaps(bboxes1, bboxes2, mode='iou', is_aligned=False, eps=1e-06):
    ...
    if is_aligned:
        ...
        if mode in ['iou', 'giou']:
            ...
            union = area1[..., None]
        if mode == 'giou':
            enclosed_lt = np.minimum(bboxes1[..., :, None, :2], bboxes2[...,
                None, :, :2])
            enclosed_rb = np.maximum(bboxes1[..., :, None, 2:], bboxes2[...,
                None, :, 2:])
    eps = np.array([eps])
...
class ATSSAssigner(object):
    ...
    def __call__(self, bboxes, num_level_bboxes, gt_bboxes,
        ...
        distances = np.sqrt(np.power(bboxes_points[:, (None), :] -
            gt_points[(None), :, :], 2).sum(-1))
        candidate_idxs = []
        start_idx = 0
        for bboxes_per_level in num_level_bboxes:
            ...
            _, topk_idxs_per_level = topk_(distances_per_level,
                selectable_k, axis=0, largest=False)
        ...
        ep_bboxes_cx = np.broadcast_to(bboxes_cx.reshape(1, -1), [num_gt,
            num_bboxes]).reshape(-1)
        ep_bboxes_cy = np.broadcast_to(bboxes_cy.reshape(1, -1), [num_gt,
            num_bboxes]).reshape(-1)
        ...
        t_ = ep_bboxes_cy[candidate_idxs].reshape(-1, num_gt) - gt_bboxes[:,
            (1)]
        r_ = gt_bboxes[:, (2)] - ep_bboxes_cx[candidate_idxs].reshape(-1,
            num_gt)
        b_ = gt_bboxes[:, (3)] - ep_bboxes_cy[candidate_idxs].reshape(-1,
            num_gt)
        ...
        assigned_gt_inds[max_overlaps != -np.inf] = argmax_overlaps[
            max_overlaps != -np.inf] + 1
        return assigned_gt_inds, max_overlaps
