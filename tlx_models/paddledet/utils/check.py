...
__all__ = ['check_gpu', 'check_npu', 'check_xpu', 'check_version',
    'check_config']
...
def check_npu(use_npu):
    ...
    cpu/gpu/xpu version.
    """
    err = """Config use_npu cannot be set as true while you are using paddlepaddle cpu/gpu/xpu version ! 
Please try: 
	1. Install paddlepaddle-npu to run model on NPU 
	2. Set use_npu as false in config file to run model on CPU/GPU/XPU"""
    try:
...
def check_xpu(use_xpu):
    ...
    cpu/gpu/npu version.
    """
    err = """Config use_xpu cannot be set as true while you are using paddlepaddle cpu/gpu/npu version ! 
Please try: 
	1. Install paddlepaddle-xpu to run model on XPU 
	2. Set use_xpu as false in config file to run model on CPU/GPU/NPU"""
    try:
...
def check_version(version='2.2'):
    ...
    """
    err = (
        """PaddlePaddle version {} or higher is required, or a suitable release/2.5 version is satisfied as well. 
Please make sure the version is good with your code."""
        .format(version))
    version_installed = [paddle_version.major, paddle_version.minor,
        paddle_version.patch, paddle_version.rc]
