"""checked by scc, time:2022-10-25"""
...
class SmoothedValue(object):
    ...
    def __str__(self):
        return self.fmt.format(median=self.median, avg=self.avg, max=self.
            max, value=self.value)
...
class TrainingStats(object):
    ...
    def update(self, stats):
        if self.meters is None:
            self.meters = {k: SmoothedValue(self.window_size) for k in
                stats.keys()}
