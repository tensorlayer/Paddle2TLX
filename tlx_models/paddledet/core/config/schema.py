...
class SchemaDict(dict):
    ...
    def find_default_keys(self):
        return [k for k in list(self.keys()) + list(self.schema.keys()) if
            self.is_default(k)]
    ...
    def find_missing_keys(self):
        missing = [k for k in self.schema.keys() if k not in self and not
            self.has_default(k)]
    ...
    def find_mismatch_keys(self):
        ...
        for arg in self.schema.values():
            if arg.type is not None:
                try:
                    check_type('{}.{}'.format(self.name, arg.name), self[
                        arg.name], arg.type)
                except Exception:
    ...
    def validate(self):
        ...
        if missing_keys:
            raise ValueError('Missing param for class<{}>: {}'.format(self.
...
def extract_schema(cls):
    ...
    schema.category = hasattr(cls, '__category__') and getattr(cls,
        '__category__') or 'module'
