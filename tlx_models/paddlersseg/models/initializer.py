...
__all__ = ['uniform_', 'normal_', 'constant_', 'ones_', 'zeros_',
    ...
    'kaiming_normal_', 'linear_init_', 'conv_init_',
    'reset_initialized_parameter']


def _no_grad_uniform_(tensor, a, b):
    tensor.set_value(paddle.uniform(shape=tensor.shape, dtype=tensor.dtype,
        min=a, max=b))
    return tensor
...
def _no_grad_fill_(tensor, value=0.0):
    tensor.set_value(tensorlayerx.constant(dtype=tensor.dtype, shape=tensor
        .shape, value=value))
    return tensor
...
def _calculate_gain(nonlinearity, param=None):
    ...
    if nonlinearity in linear_fns or nonlinearity == 'sigmoid':
        ...
        if param is None:
            negative_slope = 0.01
        elif not isinstance(param, bool) and isinstance(param, int
...
def reset_initialized_parameter(model, include_self=True):
    ...
    for _, m in model.named_sublayers(include_self=include_self):
        if isinstance(m, tensorlayerx.nn.GroupConv2d):
            k = float(m._groups) / (m._in_channels * m.kernel_size[0] * m.
                kernel_size[1])
            k = math.sqrt(k)
            ...
            if hasattr(m, 'bias') and getattr(m, 'bias') is not None:
                _no_grad_uniform_(m.bias, -k, k)
        elif isinstance(m, tensorlayerx.nn.Embedding):
            _no_grad_normal_(m.weight, mean=0.0, std=1.0)
        elif isinstance(m, (tensorlayerx.nn.BatchNorm2d, tensorlayerx.nn.
            LayerNorm)):
