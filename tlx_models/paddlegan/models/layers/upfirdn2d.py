...
def upfirdn2d_native(input, kernel, up_x, up_y, down_x, down_y, pad_x0,
    ...
    out = out.reshape([-1, 1, in_h * up_y + pad_y0 + pad_y1, in_w * up_x +
        pad_x0 + pad_x1])
    w = tensorlayerx.flip(kernel, [0, 1]).reshape((1, 1, kernel_h, kernel_w))
    out = paddle.nn.functional.conv2d(out, w)
...
def make_kernel(k):
    k = tensorlayerx.convert_to_tensor(k, dtype='float32')
    if k.ndim == 1:
...
class Upfirdn2dUpsample(nn.Module):
...
class Upfirdn2dDownsample(nn.Module):
    ...
    def forward(self, input):
        out = upfirdn2d(input, self.kernel, up=1, down=self.factor, pad=\
            self.pad)
        return out


class Upfirdn2dBlur(nn.Module):
