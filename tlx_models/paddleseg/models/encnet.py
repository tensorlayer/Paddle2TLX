...
class ENCNet(nn.Module):
    ...
    def __init__(self, num_classes, backbone, backbone_indices=[1, 2, 3],
        num_codes=32, mid_channels=512, use_se_loss=True, add_lateral=False,
        pretrained=None):
        super().__init__()
        ...
        in_channels = [self.backbone.feat_channels[index] for index in
            backbone_indices]
        self.bottleneck = layers.ConvBNReLU(in_channels[-1], mid_channels, 
            3, padding=1)
        if self.add_lateral:
            self.lateral_convs = nn.ModuleList()
            for in_ch in in_channels[:-1]:
                self.lateral_convs.append(layers.ConvBNReLU(in_ch,
                    mid_channels, 1))
            self.fusion = layers.ConvBNReLU(len(in_channels) * mid_channels,
                mid_channels, 3, padding=1)
        self.enc_module = EncModule(mid_channels, num_codes)
        self.head = nn.GroupConv2d(in_channels=mid_channels, out_channels=\
            num_classes, kernel_size=1, padding=0, data_format='channels_first'
            )
    ...
    def forward(self, inputs):
        N, C, H, W = paddle2tlx.pd2tlx.ops.tlxops.tlx_get_tensor_shape(inputs)
        ...
        if self.add_lateral:
            ...
            for j, lateral_conv in enumerate(self.lateral_convs):
                laterals.append(paddle.nn.functional.interpolate(
                    lateral_conv(feats[j]), size=paddle2tlx.pd2tlx.ops.
        ...
        out = self.head(feat)
        out = paddle.nn.functional.interpolate(out, size=[H, W], mode=\
            'bilinear', align_corners=False)
        output = [out]
        if self.training:
            fcn_out = self.fcn_head(fcn_feat)
            fcn_out = paddle.nn.functional.interpolate(fcn_out, size=[H, W],
...
class Encoding(nn.Module):
    ...
    def scaled_l2(self, x, codewords, scale):
        ...
        scaled_l2_norm = tensorlayerx.ops.multiply(reshaped_scale, (
    ...
    def aggregate(self, assignment_weights, x, codewords):
        ...
        reshaped_codewords = codewords.reshape([1, 1, num_codes, channels])
        expanded_x = tensorlayerx.tile(x.unsqueeze(2), [1, 1, num_codes, 1])
        encoded_feat = tensorlayerx.ops.multiply(assignment_weights.
            unsqueeze(3), expanded_x - reshaped_codewords).sum(axis=1)
        encoded_feat = tensorlayerx.reshape(encoded_feat, [-1, self.
            num_codes, self.channels])
        return encoded_feat
...
class EncModule(nn.Module):
    ...
    def forward(self, x):
        ...
        batch_size, _, _, _ = (paddle2tlx.pd2tlx.ops.tlxops.
