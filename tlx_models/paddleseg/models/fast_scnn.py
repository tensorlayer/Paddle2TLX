...
class FastSCNN(nn.Module):
    ...
    def __init__(self, num_classes, enable_auxiliary_loss=True,
        ...
        self.global_feature_extractor = GlobalFeatureExtractor(in_channels=\
            64, block_channels=[64, 96, 128], out_channels=128, expansion=6,
            num_blocks=[3, 3, 3], align_corners=True)
    ...
    def forward(self, x):
        logit_list = []
        input_size = paddle2tlx.pd2tlx.ops.tlxops.tlx_get_tensor_shape(x)[2:]
        higher_res_features = self.learning_to_downsample(x)
        x = self.global_feature_extractor(higher_res_features)
        x = self.feature_fusion(higher_res_features, x)
        logit = self.classifier(x)
        logit = paddle.nn.functional.interpolate(logit, input_size, mode=\
            'bilinear', align_corners=self.align_corners)
        logit_list.append(logit)
        if self.enable_auxiliary_loss:
            auxiliary_logit = self.auxlayer(higher_res_features)
            auxiliary_logit = paddle.nn.functional.interpolate(auxiliary_logit,
                input_size, mode='bilinear', align_corners=self.align_corners)
...
class LearningToDownsample(nn.Module):
    ...
    def __init__(self, dw_channels1=32, dw_channels2=48, out_channels=64):
        ...
        self.conv_bn_relu = layers.ConvBNReLU(in_channels=3, out_channels=\
            dw_channels1, kernel_size=3, stride=2)
        self.dsconv_bn_relu1 = layers.SeparableConvBNReLU(in_channels=\
            dw_channels1, out_channels=dw_channels2, kernel_size=3, stride=\
            2, padding=1)
        self.dsconv_bn_relu2 = layers.SeparableConvBNReLU(in_channels=\
            dw_channels2, out_channels=out_channels, kernel_size=3, stride=\
            2, padding=1)
...
class GlobalFeatureExtractor(nn.Module):
    ...
    def __init__(self, in_channels, block_channels, out_channels, expansion,
        ...
        self.bottleneck1 = self._make_layer(InvertedBottleneck, in_channels,
            block_channels[0], num_blocks[0], expansion, 2)
        self.bottleneck2 = self._make_layer(InvertedBottleneck,
            block_channels[0], block_channels[1], num_blocks[1], expansion, 2)
        self.bottleneck3 = self._make_layer(InvertedBottleneck,
            block_channels[1], block_channels[2], num_blocks[2], expansion, 1)
        self.ppm = layers.PPModule(block_channels[2], out_channels,
            bin_sizes=(1, 2, 3, 6), dim_reduction=True, align_corners=\
            align_corners)
...
class InvertedBottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, expansion=6, stride=2):
        ...
        self.block = nn.Sequential([layers.ConvBNReLU(in_channels=\
            in_channels, out_channels=expand_channels, kernel_size=1,
            bias_attr=False), layers.ConvBNReLU(in_channels=expand_channels,
            out_channels=expand_channels, kernel_size=3, stride=stride,
            padding=1, groups=expand_channels, bias_attr=False), layers.
            ConvBN(in_channels=expand_channels, out_channels=out_channels,
...
class FeatureFusionModule(nn.Module):
    ...
    def __init__(self, high_in_channels, low_in_channels, out_channels,
        ...
        self.dwconv = layers.ConvBNReLU(in_channels=low_in_channels,
            out_channels=out_channels, kernel_size=3, padding=1, groups=128,
            bias_attr=False)
    ...
    def forward(self, high_res_input, low_res_input):
        low_res_input = paddle.nn.functional.interpolate(low_res_input,
            paddle2tlx.pd2tlx.ops.tlxops.tlx_get_tensor_shape(
            high_res_input)[2:], mode='bilinear', align_corners=self.
            align_corners)
...
class Classifier(nn.Module):
    ...
    def __init__(self, input_channels, num_classes):
        super().__init__()
        self.dsconv1 = layers.SeparableConvBNReLU(in_channels=\
            input_channels, out_channels=input_channels, kernel_size=3,
            padding=1)
        self.dsconv2 = layers.SeparableConvBNReLU(in_channels=\
            input_channels, out_channels=input_channels, kernel_size=3,
            padding=1)
        self.conv = nn.GroupConv2d(in_channels=input_channels, out_channels
            =num_classes, kernel_size=1, padding=0, data_format=\
            'channels_first')
        self.dropout = paddle2tlx.pd2tlx.ops.tlxops.tlx_Dropout(p=0.1)
