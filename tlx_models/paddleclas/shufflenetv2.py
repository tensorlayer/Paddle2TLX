...
def channel_shuffle(x, groups):
    ...
    x = tensorlayerx.reshape(x, shape=[batch_size, groups,
        channels_per_group, height, width])
    x = tensorlayerx.transpose(x, perm=[0, 2, 1, 3, 4])
    x = tensorlayerx.reshape(x, shape=[batch_size, num_channels, height, width]
        )
    return x
...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, activation_layer=\
        nn.ReLU):
        super(InvertedResidual, self).__init__()
        self._conv_pw = ConvNormActivation(in_channels=in_channels // 2,
            out_channels=out_channels // 2, kernel_size=1, stride=1,
            padding=0, groups=1, activation_layer=activation_layer)
        self._conv_dw = ConvNormActivation(in_channels=out_channels // 2,
            out_channels=out_channels // 2, kernel_size=3, stride=stride,
            padding=1, groups=out_channels // 2, activation_layer=None)
        self._conv_linear = ConvNormActivation(in_channels=out_channels // 
            2, out_channels=out_channels // 2, kernel_size=1, stride=1,
            padding=0, groups=1, activation_layer=activation_layer)

    def forward(self, inputs):
        x1, x2 = tensorlayerx.ops.split(inputs, axis=1, num_or_size_splits=\
            [inputs.shape[1] // 2, inputs.shape[1] // 2])
        x2 = self._conv_pw(x2)
        x2 = self._conv_dw(x2)
        x2 = self._conv_linear(x2)
        out = tensorlayerx.concat([x1, x2], axis=1)
...
class InvertedResidualDS(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, activation_layer=\
        ...
        self._conv_dw_1 = ConvNormActivation(in_channels=in_channels,
            ...
            =1, groups=in_channels, activation_layer=None)
        self._conv_linear_1 = ConvNormActivation(in_channels=in_channels,
            out_channels=out_channels // 2, kernel_size=1, stride=1,
            padding=0, groups=1, activation_layer=activation_layer)
        ...
        self._conv_dw_2 = ConvNormActivation(in_channels=out_channels // 2,
            out_channels=out_channels // 2, kernel_size=3, stride=stride,
            padding=1, groups=out_channels // 2, activation_layer=None)
        self._conv_linear_2 = ConvNormActivation(in_channels=out_channels //
            2, out_channels=out_channels // 2, kernel_size=1, stride=1,
            padding=0, groups=1, activation_layer=activation_layer)
...
class ShuffleNetV2(nn.Module):
    ...
    def __init__(self, scale=1.0, act='relu', num_classes=1000, with_pool=True
        ...
        if scale == 0.25:
            ...
            raise NotImplementedError('This scale size:[' + str(scale) +
                '] is not implemented!')
        self._conv1 = ConvNormActivation(in_channels=3, out_channels=\
            stage_out_channels[1], kernel_size=3, stride=2, padding=1,
            activation_layer=activation_layer)
        self._max_pool = paddle2tlx.pd2tlx.ops.tlxops.tlx_MaxPool2d(kernel_size
            ...
            for i in range(num_repeat):
                if i == 0:
                    block = self.add_sublayer(sublayer=InvertedResidualDS(
                        ...
                        stride=2, activation_layer=activation_layer), name=\
                        str(stage_id + 2) + '_' + str(i + 1))
                else:
                    block = self.add_sublayer(sublayer=InvertedResidual(
                        in_channels=stage_out_channels[stage_id + 2],
                        out_channels=stage_out_channels[stage_id + 2],
                        stride=1, activation_layer=activation_layer), name=\
                        str(stage_id + 2) + '_' + str(i + 1))
                self._block_list.append(block)
        self._last_conv = ConvNormActivation(in_channels=stage_out_channels
            [-2], out_channels=stage_out_channels[-1], kernel_size=1,
            stride=1, padding=0, activation_layer=activation_layer)
        if with_pool:
            self._pool2d_avg = AdaptiveAvgPool2d(1, data_format=\
                'channels_first')
        if num_classes > 0:
            self._out_c = stage_out_channels[-1]
            self._fc = Linear(in_features=stage_out_channels[-1],
    ...
    def forward(self, inputs):
        ...
        if self.num_classes > 0:
            x = tensorlayerx.flatten(x, start_axis=1, stop_axis=-1)
            x = self._fc(x)
        return x
...
def shufflenet_v2_x0_25(pretrained=False, **kwargs):
    ...
    """
    return _shufflenet_v2('shufflenet_v2_x0_25', scale=0.25, pretrained=\
        pretrained, **kwargs)
...
def shufflenet_v2_x0_33(pretrained=False, **kwargs):
    ...
    """
    return _shufflenet_v2('shufflenet_v2_x0_33', scale=0.33, pretrained=\
        pretrained, **kwargs)
...
def shufflenet_v2_x0_5(pretrained=False, **kwargs):
    ...
    """
    return _shufflenet_v2('shufflenet_v2_x0_5', scale=0.5, pretrained=\
        pretrained, **kwargs)
...
def shufflenet_v2_x1_0(pretrained=False, **kwargs):
    ...
    """
    return _shufflenet_v2('shufflenet_v2_x1_0', scale=1.0, pretrained=\
        pretrained, **kwargs)
...
def shufflenet_v2_x1_5(pretrained=False, **kwargs):
    ...
    """
    return _shufflenet_v2('shufflenet_v2_x1_5', scale=1.5, pretrained=\
        pretrained, **kwargs)
...
def shufflenet_v2_x2_0(pretrained=False, **kwargs):
    ...
    """
    return _shufflenet_v2('shufflenet_v2_x2_0', scale=2.0, pretrained=\
        pretrained, **kwargs)
...
def shufflenet_v2_swish(pretrained=False, **kwargs):
    ...
    """
    return _shufflenet_v2('shufflenet_v2_swish', scale=1.0, act='swish',
