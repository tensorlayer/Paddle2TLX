...
class TheseusLayer(nn.Module):
    ...
    def init_res(self, stages_pattern, return_patterns=None, return_stages=None
        ...
        if return_patterns and return_stages:
            msg = (
                f"The 'return_patterns' would be ignored when 'return_stages' is set."
                )
            return_stages = None
        ...
        if isinstance(return_stages, list):
            if max(return_stages) > len(stages_pattern) or min(return_stages
                ) < 0:
                msg = (
                    f"The 'return_stages' set error. Illegal value(s) have been ignored. The stages' pattern list is {stages_pattern}."
                    )
                return_stages = [val for val in return_stages if val >= 0 and
                    val < len(stages_pattern)]
    ...
    def upgrade_sublayer(self, layer_name_pattern, handle_func) ->Dict[str,
        tensorlayerx.nn.Module]:
        ...
        for pattern in layer_name_pattern:
            ...
            if not layer_list:
                continue
            sub_layer_parent = layer_list[-2]['layer'] if len(layer_list
                ) > 1 else self
    ...
    def stop_after(self, stop_layer_name: str) ->bool:
        ...
        for layer_dict in layer_list:
            ...
            if not set_identity(parent_layer, name, index_list):
                msg = (
                    f"Failed to set the layers that after stop_layer_name('{stop_layer_name}') to IdentityLayer. The error layer's name is '{name}'."
                    )
                return False
            parent_layer = layer_dict['layer']
        return True

    def update_res(self, return_patterns) ->Dict[str, tensorlayerx.nn.Module]:
        ...
        hit_layer_pattern_list = self.upgrade_sublayer(return_patterns,
            handle_func=handle_func)
        if hasattr(self, 'hook_remove_helper'):
            self.hook_remove_helper.remove()
        self.hook_remove_helper = self.register_forward_post_hook(self.
            _return_dict_hook)
        return hit_layer_pattern_list
...
def set_identity(parent_layer, layer_name, layer_index_list) ->bool:
    ...
    for sub_layer_name in parent_layer._sub_layers:
        if stop_after:
            parent_layer._sub_layers[sub_layer_name] = tlx_Identity()
            continue
    ...
    if layer_index_list and stop_after:
        ...
        for num, layer_index in enumerate(layer_index_list):
            ...
            for sub_layer_index in layer_container._sub_layers:
                if stop_after:
                    parent_layer._sub_layers[layer_name][sub_layer_index
                        ] = tlx_Identity()
                    continue
...
def parse_pattern_str(pattern, parent_layer):
    ...
    while len(pattern_list) > 0:
        if '[' in pattern_list[0]:
            ...
            target_layer_index_list = list(index.split(']')[0] for index in
                pattern_list[0].split('[')[1:])
        else:
        ...
        if target_layer is None:
            msg = (
                f"Not found layer named('{target_layer_name}') specifed in pattern('{pattern}')."
                )
            return None
        if target_layer_index_list:
            for target_layer_index in target_layer_index_list:
                if int(target_layer_index) < 0 or int(target_layer_index
                    ) >= len(target_layer):
                    msg = (
                        f"Not found layer by index('{target_layer_index}') specifed in pattern('{pattern}'). The index should < {len(target_layer)} and > 0."
                        )
                    return None
                target_layer = target_layer[target_layer_index]
        layer_list.append({'layer': target_layer, 'name': target_layer_name,
