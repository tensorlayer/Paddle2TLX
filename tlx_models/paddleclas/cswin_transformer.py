...
class PatchEmbedding(nn.Module):
    ...
    def __init__(self, patch_stride=4, in_channels=3, embed_dim=96):
        super().__init__()
        self.patch_embed = nn.GroupConv2d(in_channels=in_channels,
            out_channels=embed_dim, kernel_size=7, stride=patch_stride,
            padding=2, data_format='channels_first')
        self.norm = nn.LayerNorm(embed_dim)

    def forward(self, x):
        x = self.patch_embed(x)
        x = x.flatten(start_axis=2, stop_axis=-1)
        x = x.transpose([0, 2, 1])
        x = self.norm(x)
        return x
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features, dropout):
        ...
        self.dropout = paddle2tlx.pd2tlx.ops.tlxops.tlx_Dropout(dropout)
...
def img2windows(img, h_split, w_split):
    ...
    out = out.transpose([0, 2, 4, 3, 5, 1])
    out = out.reshape([-1, h_split * w_split, C])
    return out
...
def windows2img(img_splits, h_split, w_split, img_h, img_w):
    ...
    """
    B = tensorlayerx.convert_to_tensor(img_splits.shape[0] // (img_h //
        h_split * img_w // w_split), 'int32')
    img = img_splits.reshape([B, img_h // h_split, img_w // w_split,
        h_split, w_split, img_splits.shape[-1]])
    img = img.transpose([0, 1, 3, 2, 4, 5])
    img = img.reshape([B, img_h, img_w, img_splits.shape[-1]])
    return img
...
class LePEAttention(nn.Module):
    ...
    def __init__(self, dim, resolution, h_split=7, w_split=7, num_heads=8,
        attention_dropout=0.0, dropout=0.0, qk_scale=None):
        super().__init__()
        self.dim = dim
        ...
        self.get_v = nn.GroupConv2d(in_channels=dim, out_channels=dim,
            kernel_size=3, stride=1, padding=1, n_group=dim, data_format=\
            'channels_first')
        self.softmax = nn.Softmax(axis=-1)
        self.attn_dropout = paddle2tlx.pd2tlx.ops.tlxops.tlx_Dropout(
            attention_dropout)
    ...
    def im2cswin(self, x):
        ...
        x = x.reshape([-1, self.h_split * self.w_split, self.num_heads,
            self.dim_head])
    ...
    def get_lepe(self, x, func):
        ...
        x = x.reshape([-1, C, h_split, w_split])
        lepe = func(x)
        lepe = lepe.reshape([-1, self.num_heads, self.dim_head, h_split *
            w_split])
        lepe = lepe.transpose([0, 1, 3, 2])
    ...
    def forward(self, q, k, v):
        ...
        q = q * self.scale
        attn = tensorlayerx.ops.matmul(q, k, transpose_b=True)
        attn = self.softmax(attn)
        attn = self.attn_dropout(attn)
        z = tensorlayerx.ops.matmul(attn, v)
        z = z + lepe
...
class CSwinBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, split_size=7,
        mlp_ratio=4.0, qkv_bias=False, qk_scale=None, attention_dropout=0.0,
        dropout=0.0, droppath=0.0, split_heads=True):
        super().__init__()
        self.dim = dim
        ...
        if split_heads:
            splits = [self.input_resolution[0], self.split_size]
        else:
            splits = [self.input_resolution[0], self.input_resolution[0]]
        for _ in range(num_branches):
            attn = LePEAttention(dim=dim // num_branches, resolution=\
                input_resolution, h_split=splits[0], w_split=splits[1],
                num_heads=num_heads // num_branches, qk_scale=qk_scale,
        ...
        self.drop_path = DropPath(droppath
            ) if droppath > 0.0 else tlx_Identity()
...
class MergeBlock(nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        super().__init__()
        self.conv = nn.GroupConv2d(in_channels=dim_in, out_channels=dim_out,
            kernel_size=3, stride=2, padding=1, data_format='channels_first')
...
class CSwinStage(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, split_size,
        mlp_ratio=4.0, qkv_bias=True, qk_scale=None, dropout=0.0,
        attention_dropout=0.0, droppath=0.0, last_stage=False):
        super().__init__()
        self.blocks = nn.ModuleList()
        for i in range(depth):
            block = CSwinBlock(dim=dim, input_resolution=input_resolution,
                num_heads=num_heads, split_size=split_size, mlp_ratio=\
                mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                attention_dropout=attention_dropout, dropout=dropout,
                droppath=droppath[i] if isinstance(droppath, list) else
                droppath, split_heads=not last_stage)
            self.blocks.append(copy.deepcopy(block))
        self.merge = MergeBlock(dim_in=dim, dim_out=dim * 2
            ) if not last_stage else tlx_Identity()
...
class CSwinTransformer(nn.Module):
    ...
    def __init__(self, image_size=224, patch_stride=4, in_channels=3,
        class_num=1000, embed_dim=96, depths=[2, 4, 32, 2], splits=[1, 2, 7,
        7], num_heads=[4, 8, 16, 32], mlp_ratio=4.0, qkv_bias=True,
        qk_scale=None, dropout=0.0, attention_dropout=0.0, droppath=0.0):
        super().__init__()
        self.patch_embedding = PatchEmbedding(patch_stride=patch_stride,
            in_channels=in_channels, embed_dim=embed_dim)
        depth_decay = [x.item() for x in paddle2tlx.pd2tlx.ops.tlxops.
            tlx_linspace(0, droppath, sum(depths))]
        dim = embed_dim
        resolution = image_size // 4
        self.stages = nn.ModuleList()
        num_stages = len(depths)
        for stage_idx in range(num_stages):
            stage = CSwinStage(dim=dim, input_resolution=resolution, depth=\
                depths[stage_idx], num_heads=num_heads[stage_idx],
                split_size=splits[stage_idx], mlp_ratio=mlp_ratio, qkv_bias
                =qkv_bias, qk_scale=qk_scale, dropout=dropout,
                attention_dropout=attention_dropout, droppath=depth_decay[
        ...
        self.head = nn.Linear(in_features=dim, out_features=class_num)
    ...
    def forward_features(self, x):
        ...
        for stage in self.stages:
            x = stage(x)
        x = self.norm(x)
        return tensorlayerx.reduce_mean(x, axis=1)
...
def _CSwintransformer_thiny(arch, pretrained, **kwargs):
    model = CSwinTransformer(image_size=224, embed_dim=64, depths=[1, 2, 21,
        1], splits=[1, 2, 7, 7], num_heads=[2, 4, 8, 16], droppath=0.2, **
        kwargs)
    if pretrained:
...
def CSwintransformer_thiny(pretrained=False, **kwargs):
    return _CSwintransformer_thiny('CSWinTransformer_tiny_224', pretrained,
        **kwargs)
