...
class ConvBNLayer(nn.Module):
    ...
    def __init__(self, input_channels, output_channels, filter_size, stride,
        ...
        bn_name = name + '.bn'
        self._bn = BatchNorm(act='relu', moving_mean_init=tensorlayerx.
            initializers.xavier_uniform(), moving_var_init=tensorlayerx.
            initializers.xavier_uniform(), num_features=output_channels,
            data_format='channels_first')
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, input_channels, output_channels, name=None):
        ...
        self._conv1 = ConvBNLayer(input_channels, output_channels, 1, 1, 0,
            name=name + '.0')
        self._conv2 = ConvBNLayer(output_channels, output_channels * 2, 3, 
            1, 1, name=name + '.1')

    def forward(self, inputs):
        x = self._conv1(inputs)
        x = self._conv2(x)
        return tensorlayerx.add(value=inputs, bias=x)
...
class DarkNet(nn.Module):
    ...
    def __init__(self, class_num=1000):
        ...
        self._downsample_2 = ConvBNLayer(256, 512, 3, 2, 1, name=\
            'stage.2.downsample')
        ...
        self._downsample_3 = ConvBNLayer(512, 1024, 3, 2, 1, name=\
            'stage.3.downsample')
        ...
        self._out = Linear(in_features=1024, out_features=class_num, b_init
