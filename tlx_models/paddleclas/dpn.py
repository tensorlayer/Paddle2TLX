...
class ConvBNLayer(nn.Module):
    ...
    def __init__(self, num_channels, num_filters, filter_size, stride=1,
        pad=0, groups=1, act='relu', name=None):
        ...
        self._conv = GroupConv2d(in_channels=num_channels, out_channels=\
            ...
            pad, W_init=xavier_uniform(), b_init=False, n_group=groups,
...
class DualPathFactory(nn.Module):

    def __init__(self, num_channels, num_1x1_a, num_3x3_b, num_1x1_c, inc,
        G, _type='normal', name=None):
        ...
        if _type == 'proj':
            ...
            sys.exit(1)
        data_in_ch = sum(num_channels) if isinstance(num_channels, list
            ) else num_channels
        if self.has_proj:
            self.c1x1_w_func = BNACConvLayer(num_channels=data_in_ch,
                num_filters=num_1x1_c + 2 * inc, filter_size=(1, 1), pad=(0,
                0), stride=(key_stride, key_stride), name=name + '_match')
        self.c1x1_a_func = BNACConvLayer(num_channels=data_in_ch,
            num_filters=num_1x1_a, filter_size=(1, 1), pad=(0, 0), name=\
            name + '_conv1')
        self.c3x3_b_func = BNACConvLayer(num_channels=num_1x1_a,
            num_filters=num_3x3_b, filter_size=(kw, kh), pad=(pw, ph),
            stride=(key_stride, key_stride), groups=G, name=name + '_conv2')
        self.c1x1_c_func = BNACConvLayer(num_channels=num_3x3_b,
            num_filters=num_1x1_c + inc, filter_size=(1, 1), pad=(0, 0),
            name=name + '_conv3')

    def forward(self, input):
        if isinstance(input, list):
            data_in = tensorlayerx.concat([input[0], input[1]], axis=1)
        else:
            data_in = input
        if self.has_proj:
            c1x1_w = self.c1x1_w_func(data_in)
            data_o1, data_o2 = tensorlayerx.ops.split(c1x1_w, axis=1,
                num_or_size_splits=[self.num_1x1_c, 2 * self.inc])
        else:
            data_o1 = input[0]
            data_o2 = input[1]
        ...
        c1x1_c1, c1x1_c2 = tensorlayerx.ops.split(c1x1_c, axis=1,
            num_or_size_splits=[self.num_1x1_c, self.inc])
        summ = tensorlayerx.add(value=data_o1, bias=c1x1_c1)
        dense = tensorlayerx.concat([data_o2, c1x1_c2], axis=1)
        return [summ, dense]
...
class DPN(nn.Module):
    ...
    def __init__(self, layers=68, class_num=1000):
        ...
        self.k_sec = k_sec
        self.conv1_x_1_func = ConvBNLayer(num_channels=3, num_filters=\
            init_num_filter, filter_size=init_filter_size, stride=2, pad=\
            init_padding, act='relu', name='conv1')
        ...
        for gc in range(4):
            ...
            self.dpn_func_list.append(self.add_sublayer('dpn{}'.format(
                match), DualPathFactory(num_channels=num_channel_dpn,
                num_1x1_a=R, num_3x3_b=R, num_1x1_c=bw, inc=inc, G=G, _type
                =_type1, name='dpn' + str(match))))
            ...
            for i_ly in range(2, k_sec[gc] + 1):
                ...
                if num in match_list:
                    num += 1
                self.dpn_func_list.append(self.add_sublayer('dpn{}'.format(
                    num), DualPathFactory(num_channels=num_channel_dpn,
                    num_1x1_a=R, num_3x3_b=R, num_1x1_c=bw, inc=inc, G=G,
                    _type=_type2, name='dpn' + str(num))))
                num_channel_dpn = [num_channel_dpn[0], num_channel_dpn[1] + inc
                    ]
        out_channel = sum(num_channel_dpn)
        self.conv5_x_x_bn = BatchNorm(act='relu', moving_mean_init=\
            tensorlayerx.initializers.xavier_uniform(), moving_var_init=\
            tensorlayerx.initializers.xavier_uniform(), num_features=sum(
            num_channel_dpn), data_format='channels_first')
        self.pool2d_avg = AdaptiveAvgPool2d(1, data_format='channels_first')
        stdv = 0.01
        self.out = Linear(in_features=out_channel, out_features=class_num,
    ...
    def get_net_args(self, layers):
        ...
        net_arg = {'k_r': k_r, 'G': G, 'k_sec': k_sec, 'inc_sec': inc_sec,
            'bw': bw, 'r': r}
    ...
    def forward(self, input):
        ...
        for gc in range(4):
            ...
            for i_ly in range(2, self.k_sec[gc] + 1):
                convX_x_x = self.dpn_func_list[dpn_idx](convX_x_x)
                dpn_idx += 1
        conv5_x_x = tensorlayerx.concat(convX_x_x, axis=1)
...
def _dpn(arch, layers, pretrained, **kwargs):
    model = DPN(layers=layers, **kwargs)
    if pretrained:
...
def dpn68(pretrained=False, **kwargs):
    return _dpn('dpn68', 68, pretrained, **kwargs)


def dpn107(pretrained=False, **kwargs):
    return _dpn('dpn107', 107, pretrained, **kwargs)
