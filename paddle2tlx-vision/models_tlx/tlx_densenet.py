...
from tensorlayerx.nn import Conv2d, BatchNorm, Linear, Dropout
...
class BNACConvLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self._batch_norm = BatchNorm(num_features=num_channels, act=act, data_format='channels_first')
...
class DenseLayer(nn.Module):
    ...
    def __init__(self, num_channels, growth_rate, bn_size, dropout):
        ...
        if dropout:
            ...
            self.dropout_func = Dropout(p=dropout, name="downscale_in_infer")
    ...
    def forward(self, input):
        ...
        conv = tlx.ops.concat([input, conv], axis=1)
        return conv
            ...
            self.dense_layer_func.append(
                self.add_sublayer(
                    ...
                    DenseLayer(num_channels=pre_channel,
                               ...
                               bn_size=bn_size,
                               dropout=dropout)))  # TODO - paddle/fluid/dygraph/layers -> nn.Layer -> add_sublayer
...
class TransitionLayer(nn.Module):
    ...
    def __init__(self, num_channels, num_output_features):
        ...
        self.pool2d_avg = AvgPool2d(kernel_size=2, stride=2, padding=0, data_format='channels_first')
...
class ConvBNLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self._batch_norm = BatchNorm(num_features=num_filters, act=act, data_format='channels_first')  # TODO
...
class DenseNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.pool2d_max = MaxPool2d(kernel_size=3, stride=2, padding=1, data_format='channels_first')
        ...
        self.batch_norm = BatchNorm(num_features=num_features, act="relu", data_format='channels_first')
        if self.with_pool:
            # self.pool2d_avg = AdaptiveAvgPool2D(1)
            self.pool2d_avg = AdaptiveAvgPool2d(output_size=1, data_format='channels_first')
