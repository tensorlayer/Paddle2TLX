...
def _drop_connect(inputs, prob, is_test):
    if is_test:
        ...
        keep_prob = 1.0 - prob
        inputs_shape = tensorlayerx.ops.get_tensor_shape(inputs)
        # inputs_shape = paddle.shape(inputs)
        random_tensor = keep_prob + tensorlayerx.ops.random_uniform(minval=0,
            shape=(inputs_shape[0], 1, 1, 1))
        # random_tensor = keep_prob + tensorlayerx.rand(
        #     shape=[inputs_shape[0], 1, 1, 1])
        binary_tensor = tensorlayerx.floor(random_tensor)
        # binary_tensor = paddle.floor(random_tensor)
        output = tensorlayerx.multiply(inputs, binary_tensor) / keep_prob
        # output = paddle.multiply(inputs, binary_tensor) / keep_prob
    return output


class Conv2ds(tensorlayerx.nn.Module):
    def __init__(self,
                 input_channels,
                 output_channels,
                 filter_size,
...
##############################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################
            self._bn = tensorlayerx.nn.BatchNorm(
...
class ExpandConvNorm(tensorlayerx.nn.Module):
    def __init__(self,
                 input_channels,
                 block_args,
                 padding_type,
...
class DepthwiseConvNorm(tensorlayerx.nn.Module):
    def __init__(self,
                 input_channels,
                 block_args,
                 padding_type,
...
class ProjectConvNorm(tensorlayerx.nn.Module):
    def __init__(self,
                 input_channels,
                 block_args,
                 padding_type,
...
###########################################################################################################################################################################################################################################################################################################################
        self._pool = tensorlayerx.nn.AdaptiveAvgPool2d(1,data_format='channels_first')
    ...
    def forward(self, inputs):
        ...
        x = self._conv1(x)
        x = self._conv2(x)
        out = tensorlayerx.multiply(inputs, x)
        return out


class MbConvBlock(tensorlayerx.nn.Module):
    def __init__(self,
                 input_channels,
                 block_args,
                 padding_type,
    ...
    def forward(self, inputs):
        ...
        if self.id_skip and \
            ...
            if self.drop_connect_rate:
                x = _drop_connect(x, self.drop_connect_rate, not self.is_train)
            x = tensorlayerx.ops.add(x, inputs)

        return x


class ConvStemNorm(tensorlayerx.nn.Module):
    def __init__(self,
                 input_channels,
                 padding_type,
                 _global_params,
...
class ExtractFeatures(tensorlayerx.nn.Module):
    def __init__(self,
                 input_channels,
                 _block_args,
                 _global_params,
...
class EfficientNet(tensorlayerx.nn.Module):
    def __init__(self,
                 name="b0",
                 padding_type="SAME",
...
########################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################
            self._drop = tensorlayerx.nn.Dropout(
...
########################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################################
        self._fc = tensorlayerx.nn.Linear(
...
def efficientnetb1(pretrained=False, **kwargs):
    return _efficientnet('b1', pretrained, **kwargs)


def efficientnetb7(pretrained=False, **kwargs):
    return _efficientnet('b7', pretrained, **kwargs)
...
if __name__ == '__main__':
    ...
    model_tlx = efficientnetb1(pretrained=True)
    model_tlx.set_eval()
    img = load_image(image_file, "tlx")

    result_pd = model_tlx(img)
    file_path = '../../images/imagenet_classes.txt'
    with open(file_path) as f:
        classes = [line.strip() for line in f.readlines()]
    # print(result_pd)
    print(classes[np.argmax(result_pd[0])])
