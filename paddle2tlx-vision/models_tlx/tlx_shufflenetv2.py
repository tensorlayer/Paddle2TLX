...
def channel_shuffle(x, groups):
    ...
    x = tlx.ops.transpose(x, perm=[0, 2, 1, 3, 4])

    # flatten
    # x = paddle.reshape(x, shape=[batch_size, num_channels, height, width])
    x = tlx.ops.reshape(x, shape=[batch_size, num_channels, height, width])
    return x
...
class InvertedResidual(nn.Module):
    ...
    def forward(self, inputs):
        ...
        out = tlx.ops.concat([x1, x2], axis=1)
...
class ShuffleNetV2(nn.Module):
    ...
    def __init__(self, scale=1.0, act="relu", num_classes=1000, with_pool=True):
        ...
        self._max_pool = MaxPool2d(kernel_size=3, stride=2, padding=1, data_format='channels_first')
        ...
        if with_pool:
            ...
            self._pool2d_avg = AdaptiveAvgPool2d(output_size=1, data_format='channels_first')

        # 5. fc
        if num_classes > 0:
            self._out_c = stage_out_channels[-1]
            self._fc = Linear(in_features=stage_out_channels[-1], out_features=num_classes)
