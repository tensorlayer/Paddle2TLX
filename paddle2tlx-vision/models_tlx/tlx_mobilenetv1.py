...
# from utils.paddle_origin.vision.ops import ConvNormActivation
    ...
    def __init__(self, scale=1.0, num_classes=1000, with_pool=True):
        ...
        dws21 = self.add_sublayer(sublayer=DepthwiseSeparable(in_channels=int(
            32 * scale),
                                                              ...
                                                              stride=1,
                                                              scale=scale),
                                  name="conv2_1")  # TODO - tlx not add_sublayer
        ...
        if with_pool:
            ...
            self.pool2d_avg = nn.AdaptiveAvgPool2d(1, data_format='channels_first')

        if num_classes > 0:
            # self.fc = nn.Linear(int(1024 * scale), num_classes)
            self.fc = nn.Linear(out_features=num_classes, in_features=int(1024 * scale))  # TODO
