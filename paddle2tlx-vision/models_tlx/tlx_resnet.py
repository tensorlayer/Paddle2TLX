...
class BasicBlock(nn.Module):
    ...
    def __init__(self,
        ...
        #                        stride=stride,
        #                        bias_attr=False)
        self.conv1 = nn.Conv2d(out_channels=planes,
                               kernel_size=3,
                               stride=stride,
                               act=None,
                               padding=1,
        ...
        self.bn1 = norm_layer(num_features=planes, data_format='channels_first')
        self.relu = nn.ReLU()
        # self.conv2 = nn.Conv2D(planes, planes, 3, padding=1, bias_attr=False)
        self.conv2 = nn.Conv2d(out_channels=planes, kernel_size=3, act=None, padding=1,
                               b_init=False, in_channels=planes, data_format='channels_first')
        # self.bn2 = norm_layer(planes)
        self.bn2 = norm_layer(num_features=planes, data_format='channels_first')
...
class BottleneckBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(out_channels=width, kernel_size=1, padding=0, in_channels=inplanes,
                               b_init=False, data_format='channels_first')
        # self.bn1 = norm_layer(width)
        self.bn1 = norm_layer(num_features=width, data_format='channels_first')
        ...
        #                        dilation=dilation,
        #                        bias_attr=False)
        self.conv2 = nn.Conv2d(out_channels=width,
                               kernel_size=3,
                               stride=stride,
                               padding=dilation,
                               # dilation=(dilation, dilation),  # TODO
                               b_init=False,
                               in_channels=width,
                               data_format='channels_first')
        # self.bn2 = norm_layer(width)
        self.bn2 = norm_layer(num_features=width, data_format='channels_first')

        # self.conv3 = nn.Conv2D(width,
        #                        planes * self.expansion,
        #                        1,
        #                        bias_attr=False)
        self.conv3 = nn.Conv2d(out_channels=planes * self.expansion,
                               kernel_size=1,
                               padding=0,
                               b_init=False,
                               in_channels=width,
        ...
        self.bn3 = norm_layer(num_features=planes * self.expansion, data_format='channels_first')
        self.relu = nn.ReLU()
        self.downsample = downsample
        self.stride = stride
...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        #                        padding=3,
        #                        bias_attr=False)
        self.conv1 = nn.Conv2d(out_channels=self.inplanes,
                               kernel_size=7,
                               stride=2,
                               padding=3,  # TODO
                               b_init=False,
        ...
        self.bn1 = self._norm_layer(num_features=self.inplanes, data_format='channels_first')
        self.relu = nn.ReLU()
        # self.maxpool = nn.MaxPool2D(kernel_size=3, stride=2, padding=1)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1, data_format='channels_first')
        ...
        if with_pool:
            ...
            self.avgpool = nn.AdaptiveAvgPool2d((1, 1), data_format='channels_first')  # TODO

        if num_classes > 0:
            # self.fc = nn.Linear(512 * block.expansion, num_classes)
            self.fc = nn.Linear(out_features=num_classes, in_features=512 * block.expansion)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                #           stride=stride,
                #           bias_attr=False),
                nn.Conv2d(out_channels=planes * block.expansion,
                          kernel_size=1,
                          padding=0,
                          stride=stride,
                          b_init=False,
                ...
                norm_layer(num_features=planes * block.expansion, data_format='channels_first'),
            )
    ...
    def forward(self, x):
        x = self.conv1(x)  # [1, 64, 112, 112] - 一致
        x = self.bn1(x)  # TODO - 开始不一致
        x = self.relu(x)
        ...
        if self.num_classes > 0:
            ...
            x = nn.Flatten()(x)
            # print('x.numpy =', x.shape)  # TODO
            # shape = x.get_shape().as_list()
            # shape = x.shape
            # x = tlx.reshape(x, [-1, shape[1] * shape[2] * shape[3]])  # [1, 512]
            x = self.fc(x)  # [1, 1000]
