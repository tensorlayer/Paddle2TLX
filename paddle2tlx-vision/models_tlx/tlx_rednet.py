...
from .tlx_resnet import BottleneckBlock, ResNet
...
class Involution(nn.Module):
    def __init__(self, channels, kernel_size, stride):
        ...
        if stride > 1:
            ...
            self.avgpool = nn.AvgPool2d(stride, stride, data_format='channels_first', padding=0)
        ...
        self.stem = nn.Sequential([
            nn.Sequential(OrderedDict([
                ('conv', nn.Conv2d(
                    ...
                    b_init=None,
                    data_format='channels_first')),
                ('bn', nn.BatchNorm2d(num_features=self.inplanes // 2, data_format='channels_first')),
                ('activate', nn.ReLU())])
            ),
            Involution(self.inplanes // 2, 3, 1),
            nn.BatchNorm2d(num_features=self.inplanes // 2, data_format='channels_first'),
            nn.ReLU(),
            nn.Sequential(OrderedDict([
                ('conv', nn.Conv2d(
                    ...
                    b_init=None,
                    data_format='channels_first')),
                ('bn', nn.BatchNorm2d(num_features=self.inplanes, data_format='channels_first')),
                ('activate', nn.ReLU())])
            )]
        )
