...
from .ops.ops_fusion_tlx import ConvNormActivation
# from utils.load_model import restore_model
import paddle.nn as pd_nn
from .ops.tlx_pooling import pd_AvgPool2D

def get_new_dict(stat_dicts):
    new_stat_dicts = {}
    # Mixed_5c.branch3x3dbl_1.bn._variance (64,)
    # Mixed_7c.branch3x3_2b.bn._mean (384,)
    for key in stat_dicts.keys():
        if key == "fc.weight":
            new_stat_dicts[key.replace("fc.weight", "fc.weights")] = stat_dicts[key]
            continue
        elif key == "fc.bias":
            new_stat_dicts[key.replace("fc.bias", "fc.biases")] = stat_dicts[key]
            continue

        if key.endswith(".0.weight"):
                new_stat_dicts[key.replace(".0.weight", ".0.filters")] = stat_dicts[key]
                continue
        
        if key.endswith(".weight"):
            # new_stat_dicts[key.replace(".weight", ".gamma")] = stat_dicts[key]
            new_stat_dicts[key.replace(".weight", ".gamma")] = stat_dicts[key]
            continue
        if key.endswith("._mean"):
            new_stat_dicts[key.replace("._mean", ".moving_mean")] = stat_dicts[key]
            continue
        if key.endswith("._variance"):
            new_stat_dicts[key.replace("._variance", ".moving_var")] = stat_dicts[key]
            continue
        if key.endswith(".bias"):
            new_stat_dicts[key.replace(".bias", ".beta")] = stat_dicts[key]
            continue
        else:
            print("***", key)
            # new_stat_dicts[key] = stat_dicts[key]
    return new_stat_dicts
...
class InceptionStem(nn.Module):
    ...
    def __init__(self):
        ...
        self.max_pool = MaxPool2d(kernel_size=3, stride=2, padding=0, data_format='channels_first')
...
class InceptionA(nn.Module):
    ...
    def __init__(self, num_channels, pool_features):
        ...
        self.branch_pool = pd_AvgPool2D(kernel_size=3,
                                     stride=1,
                                     padding=1,
                                     exclusive=False)
...
class InceptionV3(nn.Module):
    ...
    def __init__(self, num_classes=1000, with_pool=True):
        ...
        self.inception_block_list = nn.ModuleList()  # TODO - bug
...
def inception_v3(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        print("*" * 45)
        new_param = get_new_dict(param)
        # for key in new_param.keys():
        #     print(key, new_param[key].shape)
        # print(f"new param.keys()={new_param.keys()}")
        # restore_model(param, model)
        # model.set_dict(new_param)
        model.set_state_dict(new_param)
    return model
