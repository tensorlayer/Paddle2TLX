...
class ConvBnAct(tlx.nn.Module):
    def __init__(self,
                 ...
                 dilation=1,
                 groups=1,
                 act_layer=tlx.nn.LeakyReLU,
                 norm_layer=tlx.nn.BatchNorm2d):
        super().__init__()
        if padding is None:
        ...
        self.bn = norm_layer(num_features=output_channels, data_format='channels_first')
        self.act = act_layer()

    def forward(self, inputs):  # [1,3,224,224]
        x = self.conv(inputs)
        x = self.bn(x)
...
def create_stem(in_chans=3,
    ...
    for i, out_c in enumerate(out_chs):
        ...
        layers.append((conv_name, ConvBnAct(
                in_c,
                out_c,
                kernel_size,
        ...
        last_conv = conv_name
    if pool:
        layers.append(('pool', tlx.nn.MaxPool2d(kernel_size=3, stride=2, padding=1, data_format='channels_first')))
    stem = tlx.nn.Sequential(collections.OrderedDict(layers))
    return stem, dict(
        num_chs=in_c, reduction=stride, module='.'.join(['stem', last_conv]))


class DarkBlock(tlx.nn.Module):
    def __init__(self,
                 in_chs,
                 out_chs,
                 dilation=1,
...
class CrossStage(tlx.nn.Module):
    def __init__(self,
                 in_chs,
                 out_chs,
                 stride,
        ...
        # self.blocks = tlx.nn.Sequential()
        layers = []
        for i in range(depth):
            layers.append((str(i), block_fn(prev_chs, block_out_chs, dilation, bottle_ratio,
                         groups, **block_kwargs)))
        ...
        self.blocks = tlx.nn.Sequential(collections.OrderedDict(layers))
    ...
    def forward(self, x):
        ...
        out = self.conv_transition(tlx.concat([xs, xb], axis=1))
        return out


class DarkStage(tlx.nn.Module):
    def __init__(self,
                 in_chs,
                 out_chs,
                 stride,
...
class CSPNet(tlx.nn.Module):
    def __init__(self,
        ...
        self.stages = tlx.nn.ModuleList()
        layers = []
        for i, sa in enumerate(per_stage_args):
            # stri = str(i)
            # layers.append((i, stage_fn(
        ...
        # self.stages = tlx.nn.ModuleList(layers)

        # Construct the head
        self.num_features = prev_chs

        self.pool = tlx.nn.AdaptiveAvgPool2d(1, data_format='channels_first')
        self.flatten = tlx.nn.Flatten(name='flatten')  # ======================================
        self.fc = tlx.nn.Linear(
...
def _cspdarknet53(arch, pretrained, **kwargs):
    model = CSPNet(MODEL_CFGS[arch], block_fn=DarkBlock, **kwargs)
...
def cspdarknet53(pretrained=False, **kwargs):
    return _cspdarknet53("CSPDarkNet53", pretrained, **kwargs)